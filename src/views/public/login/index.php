...
$router->get(function () use ($client) {
    ...
    if ($code) {
        if (!SOCIAL_AUTH_ENABLED) {
            MessageUtil::setMessage("Google login is currently disabled. Please sign in with email and password.");
    ...
    $seo = $seoService->seoForRoute('login', $appUrl);

    return TemplateResponse::render(__DIR__ . "/index.twig", [
        'seo' => $seo,
        'schemaJsonList' => $seoService->schemaJsonListForRoute('login', $appUrl, $seo),
    ]);
});
