...
{
    return StoreOwnerResolverService::resolve((int)($_GET['owner'] ?? 0));
}
...
function recordStoreCheckoutPaymentMethodPreferences(
    int $ownerId,
    int $clientId,
    string $providerType,
    ...
    if ($ownerId <= 0 || $clientId <= 0 || !in_array($providerType, ['stripe', 'square'], true)) {
        return;
    }
    ...
    $expMonth = null;
    $expYear = null;
    if (preg_match('/^(\d{1,2})\/(\d{2,4})$/', trim($cardExp), $matches)) {
    ...
    (new ClientPaymentMethodService())->recordFromSuccessfulPayment([
...
$router->get(function () {
    ...
    $paymentAvailability = $storeAvailable && $ownerId > 0
        ...
        : null;
    $activeProvider = $paymentAvailability['primary_provider'] ?? ($storeAvailable && $ownerId > 0 ? $providersRepo->getActiveProviderForOwner($ownerId) : null);
    $paymentAvailabilityForView = $paymentAvailability;
    if (is_array($paymentAvailabilityForView)) {
        unset($paymentAvailabilityForView['primary_provider']);
    }
...
$router->post(function () {
    ...
    $savePaymentMethod = !empty($payload['save_payment_method']);
    $autoChargeConsent = !empty($payload['auto_charge_consent']);
    ...
    $ownerId = (int)($cart->id_owner ?? 0);
    if ($ownerId <= 0) {
        $ownerId = StoreOwnerResolverService::resolve((int)($payload['owner_id'] ?? 0));
    }
    if ($ownerId <= 0) {
        echo json_encode(["success" => false, "message" => "Store owner could not be resolved."]);
        return;
    }
    if (!StorefrontAccessService::ownerCanUseStore($ownerId)) {
        echo json_encode(["success" => false, "message" => "This store is not available right now."]);
        return;
    }
    ...
    if ($action === 'summary') {
        ...
        echo json_encode([
            ...
            "cart" => [
                ...
                "total" => $total,
                "pricing" => [
                    "base_amount" => $paymentAvailability['base_amount'],
                    "base_currency" => $paymentAvailability['base_currency'],
                    "display_amount" => $paymentAvailability['display_amount'],
                    "display_currency" => $paymentAvailability['display_currency'],
                    "payment_amount" => $paymentAvailability['payment_amount'],
                    "payment_currency" => $paymentAvailability['payment_currency'],
                    "exchange_rate" => $paymentAvailability['exchange_rate'],
                    "display_exchange_rate" => $paymentAvailability['display_exchange_rate'],
                    "exchange_rate_source" => $paymentAvailability['exchange_rate_source'],
                ],
            ...
            "payment_availability" => [
                "available_methods" => $paymentAvailability['available_methods'],
                "unavailable_methods" => $paymentAvailability['unavailable_methods'],
                "bank_transfer_available" => $paymentAvailability['bank_transfer_available'],
                "primary_provider_type" => $paymentAvailability['primary_provider_type'],
            ],
    ...
    $providersRepo = new PaymentProvidersRepository();
    $activeProvider = $paymentAvailability['primary_provider'] ?? $providersRepo->getActiveProviderForOwner($ownerId);
    $providerType = $activeProvider ? (string)($activeProvider->provider_type ?? '') : '';
    $requestedPaymentMethod = strtolower(trim((string)($payload['payment_method'] ?? $providerType)));
    $isBankTransfer = $requestedPaymentMethod === 'bank_transfer';

    if (!$isBankTransfer && (!$activeProvider || !in_array($providerType, ['square', 'stripe', 'paypal'], true))) {
        echo json_encode([
            "success" => false,
            "message" => "No card or wallet payment provider is available for this country/currency. Bank transfer is available."
        ]);
        return;
    }
    ...
    if ($action === 'paypal_create_order') {
        if ($providerType !== 'paypal') {
            echo json_encode(["success" => false, "message" => "PayPal is not the active payment provider."]);
            return;
        }
        try {
            $paypal = PaymentProviderFactory::create($activeProvider);
            $paypalOrder = $paypal->createOrder($total, [
                'description' => "Store Order - Cart #" . $cart->id,
                'return_url' => rtrim((string)($_ENV['APP_URL'] ?? ''), '/') . '/store/checkout',
                'cancel_url' => rtrim((string)($_ENV['APP_URL'] ?? ''), '/') . '/store/cart',
            ]);
            echo json_encode($paypalOrder
                ? ["success" => true, "order_id" => (string)$paypalOrder->id]
                : ["success" => false, "message" => "PayPal order could not be created."]);
        } catch (\Throwable $e) {
    ...
    if (!$isBankTransfer && $customerToken === '') {
        echo json_encode([
            "success" => false,
            "message" => "Payment token missing."
        ]);
    ...
    if (!(new StoreProductsRepository())->hasStockForItems($cartItems)) {
        echo json_encode(["success" => false, "message" => "One or more products no longer have enough stock. Please review your cart."]);
        return;
    }
    ...
    if ($isBankTransfer) {
        ...
        $paymentCurrency = $displayCurrencyForRecord;
    }
    ...
    if ($isBankTransfer) {
        $paymentResponse = [
            'success' => true,
            'payment_id' => 'BT-' . strtoupper(bin2hex(random_bytes(6))),
        ...
        try {
            $paypal = PaymentProviderFactory::create($activeProvider);
            $charge = $paypal->chargeCustomer($customerToken, $paymentAmount, ['description' => "Store Order - Cart #" . $cart->id]);
            $paymentResponse = $charge ? [
                'success' => true,
                'payment_id' => $charge->id ?? null,
                'reference' => $charge->id ?? null,
                'raw' => json_encode($charge->raw ?? $charge)
            ] : ['success' => false, 'message' => 'PayPal payment could not be captured.'];
        } catch (\Throwable $e) {
            $paymentResponse = ['success' => false, 'message' => $e->getMessage()];
        }
    }
    ...
    $paymentData = [
        ...
        'status' => $isBankTransfer ? StorePaymentsRepository::STATUS_PENDING : StorePaymentsRepository::STATUS_PAID,
        ...
        'payment_currency' => strtoupper($paymentCurrency),
        'exchange_rate' => (float)($paymentAvailability['exchange_rate'] ?? 1),
        'exchange_rate_source' => (string)($paymentAvailability['exchange_rate_source'] ?? ''),
        'provider_type' => $providerType,
        'payment_provider_id' => $activeProvider ? (int)($activeProvider->id ?? 0) : null,
        'manual_review_status' => $isBankTransfer ? 'PENDING' : null,
        'id_user_business' => $ownerId,
        'site_key' => (string)($cart->site_key ?? ''),
    ];

    $paymentSaved = $paymentsRepo->addCompatible($paymentData);
    ...
    if (!$isBankTransfer) {
        ...
        (new StoreProductsRepository())->decrementStockForItems($cartItems);
    }
    ...
    if ($isBankTransfer) {
        echo json_encode([
            "success" => true,
            "message" => "Bank transfer request created. Payment is pending manual review.",
            "redirect" => \App\Utils\LocationUtils::pathFor("store/order-access?token=" . urlencode((string)$order->public_token))
        ]);
        return;
    }
        ...
        try {
            ...
            if ($stripeCustomerId) {
                recordStoreCheckoutPaymentMethodPreferences(
                    ...
                    (string)$guestEmail,
                    (int)$orderId,
                    isset($paymentId) ? (int)$paymentId : null,
                    (bool)$savePaymentMethod,
                    (bool)$autoChargeConsent
                );
            ...
            if ($squareCustomer && !empty($squareCustomer['id'])) {
                ...
                if ($squareCard && !empty($squareCard['id'])) {
                    ...
                    recordStoreCheckoutPaymentMethodPreferences(
                        ...
                        (int)$userId,
                        (string)$providerType,
                        (string)$squareCustomer['id'],
                        (string)$squareCard['id'],
                        (string)$squareCard['id'],
                        (string)($cardBrand !== '' ? $cardBrand : 'square'),
                        (string)$cardLast4,
                        (string)$cardExp,
                        (string)$guestName,
                        (string)$guestEmail,
