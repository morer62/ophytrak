...
$router->post(function () {
    ...
    if ($docRepo->getByType((int)$parentOrder->id, 'contract_signed')) {
        LocationUtils::redirectInternal("order-access/suborder?token=" . urlencode($token) . "&signed=1&t=" . time());
        return;
    }
    ...
    $redirectUrl = "order-access/suborder?token=" . urlencode($token) . "&signed=1&t=" . time();
