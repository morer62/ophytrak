...
$router->post(function () {
    ...
    if (!$cardToken || !$customerEmail) {
        LocationUtils::redirectInternal("order-access/suborder/full?token=" . urlencode($token) . "&payment_error=missing_payment_data&t=" . time()); return;
    }

    $paymentRequestLabel = TranslationService::trans('planner_hub.suborder_full_payment', ['suborder_id' => $suborder->id]);
    $charge = (new OrderAccessPaymentMethodService())->chargeOrderAccessPayment($parentOrder, (int)$paymentOwnerId, $activeProvider, $totalAmount, $cardToken, [
        'note' => $paymentRequestLabel,
        'reference_id' => 'Suborder-' . $suborderId,
        'customer_email' => $customerEmail,
    ], $_POST, ['source' => 'order_access_suborder_full', 'order_id' => (int)$parentOrder->id, 'suborder_id' => (int)$suborderId]);
