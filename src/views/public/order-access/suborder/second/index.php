...
$router->get(function () {
    ...
    $paymentMethodContext = (new OrderAccessPaymentMethodService())->buildViewContext($parentOrder, (int)$paymentOwnerId, $activeProvider);
    ...
    return TemplateResponse::render(__DIR__ . "/index.twig", [
        ...
        "payment_error" => isset($_GET["payment_error"]) ? TranslationService::trans("planner_hub.missing_payment_data") : null,
...
$router->post(function () {
    ...
    if (!$cardToken || !$customerEmail || ($activeProvider->provider_type === 'stripe' && $billingZip === '')) {
        LocationUtils::redirectInternal("order-access/suborder/second?token=" . urlencode($token) . "&payment_error=missing_payment_data&t=" . time()); return;
    }

    $paymentRequestLabel = TranslationService::trans('planner_hub.suborder_final_payment', ['suborder_id' => $suborder->id]);
    $charge = (new OrderAccessPaymentMethodService())->chargeOrderAccessPayment($parentOrder, (int)$paymentOwnerId, $activeProvider, $secondAmount, $cardToken, [
        'note' => $paymentRequestLabel,
        'reference_id' => 'Suborder-' . $suborderId,
        'customer_email' => $customerEmail,
        'billing_address' => $billingAddress,
        'billing_zip' => $billingZip,
    ], $_POST, ['source' => 'order_access_suborder_second', 'order_id' => (int)$parentOrder->id, 'suborder_id' => (int)$suborderId]);

    if ($charge === false) {
        return TemplateResponse::render(__DIR__ . "/error.twig", [
            "error" => TranslationService::trans('planner_hub.payment_could_not_be_processed')
        ]);
    }
