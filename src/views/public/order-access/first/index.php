...
$router->post(function () {
    ...
    if (!$cardToken || !$customerEmail) {
        ...
        LocationUtils::redirectInternal("order-access/first?token=" . urlencode($token) . "&payment_error=missing_payment_data&t=" . time()); return;
    }

    $paymentRequestLabel = TranslationService::trans('planner_hub.order_first_payment', ['order_id' => $order->id]);
    $charge = (new OrderAccessPaymentMethodService())->chargeOrderAccessPayment($order, (int)$paymentOwnerId, $activeProvider, $firstAmount, $cardToken, [
        'note' => $paymentRequestLabel,
        'reference_id' => 'VNV-341' . $order->id,
        'customer_email' => $customerEmail,
        'billing_address' => $billingAddress,
    ], $_POST, ['source' => 'order_access_first', 'order_id' => (int)$order->id]);
