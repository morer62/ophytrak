...
$router->post(function () {
    ...
    if (!$cardToken || !$customerEmail) {
        ...
        LocationUtils::redirectInternal("order-access/full?token=" . urlencode($token) . "&payment_error=missing_payment_data&t=" . time()); return;
    }

    $paymentRequestLabel = TranslationService::trans('planner_hub.order_full_payment', ['order_id' => $order->id]);
    $charge = (new OrderAccessPaymentMethodService())->chargeOrderAccessPayment($order, (int)$paymentOwnerId, $activeProvider, $totalAmount, $cardToken, [
        'note' => $paymentRequestLabel,
        'reference_id' => 'VNV-341' . $order->id,
        'customer_email' => $customerEmail,
        'billing_address' => $billingAddress,
    ], $_POST, ['source' => 'order_access_full', 'order_id' => (int)$order->id]);
