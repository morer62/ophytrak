...
use App\Services\OrderAccessPaymentMethodService;
...
$router->get(function () {
    ...
    $paymentMethodContext = (new OrderAccessPaymentMethodService())->buildViewContext($order, (int)$paymentOwnerId, $activeProvider);
    ...
    return TemplateResponse::render(__DIR__ . "/index.twig", [
        ...
        "client_is_authenticated" => $paymentMethodContext["client_is_authenticated"] ?? false,
        "client_email" => $paymentMethodContext["client_email"] ?? "",
        "client_name" => $paymentMethodContext["client_name"] ?? "",
        "saved_payment_methods" => $paymentMethodContext["saved_payment_methods"] ?? [],
        "gateway_supports_saved_methods" => $paymentMethodContext["gateway_supports_saved_methods"] ?? false,
        "has_saved_payment_methods" => $paymentMethodContext["has_saved_payment_methods"] ?? false,
        "payment_error" => isset($_GET["payment_error_message"]) ? urldecode((string)$_GET["payment_error_message"]) : (isset($_GET["payment_error"]) ? TranslationService::trans("planner_hub.missing_payment_data") : null),
...
$router->post(function () {
    ...
    if ($amountInput > $secondAmount) {
        ...
        return TemplateResponse::render(__DIR__ . "/error.twig", ["error" => TranslationService::trans('planner_hub.amount_cannot_exceed_second_payment', ['amount' => number_format($secondAmount, 2)])]);
    }
    ...
    if (!$cardToken || !$customerEmail || ($activeProvider->provider_type === 'stripe' && $billingZip === '')) {
        ...
        LocationUtils::redirectInternal("order-access/second?token=" . urlencode($token) . "&payment_error=missing_payment_data&t=" . time()); return;
    }

    $paymentRequestLabel = TranslationService::trans('planner_hub.order_final_installment', ['order_id' => $order->id]);
    $charge = (new OrderAccessPaymentMethodService())->chargeOrderAccessPayment($order, (int)$paymentOwnerId, $activeProvider, $amountInput, $cardToken, [
        'note' => $paymentRequestLabel,
        'reference_id' => 'VNV-341' . $order->id,
        'customer_email' => $customerEmail,
        'billing_address' => $billingAddress,
        'billing_zip' => $billingZip,
    ], $_POST, ['source' => 'order_access_second', 'order_id' => (int)$order->id]);

    if ($charge === false) { LocationUtils::redirectInternal("order-access/second?token=" . urlencode($token) . "&payment_error_message=" . urlencode(TranslationService::trans('planner_hub.payment_could_not_be_processed')) . "&t=" . time()); return; }
    if (isset($charge->status) && $charge->status === 'payment_failed') { $errorMessage = $charge->_error_details['message'] ?? TranslationService::trans('planner_hub.payment_failed'); LocationUtils::redirectInternal("order-access/second?token=" . urlencode($token) . "&payment_error_message=" . urlencode($errorMessage) . "&t=" . time()); return; }
    if (empty($charge->paid) && (empty($charge->status) || !in_array($charge->status, ['completed', 'succeeded', 'COMPLETED'], true))) { $errorMessage = TranslationService::trans('planner_hub.payment_was_not_completed', ['status' => $charge->status ?? 'unknown']); LocationUtils::redirectInternal("order-access/second?token=" . urlencode($token) . "&payment_error_message=" . urlencode($errorMessage) . "&t=" . time()); return; }
