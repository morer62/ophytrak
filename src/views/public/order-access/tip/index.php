...
if (!$cardToken || empty($customerEmail) || $tipAmount < 1) {
    $missingFields = [];
...
try {
    $charge = (new OrderAccessPaymentMethodService())->chargeOrderAccessPayment($order, (int)$paymentOwnerId, $activeProvider, $tipAmount, $cardToken, [
        'note' => TranslationService::trans('planner_hub.gratuity_tip_order', ['order_id' => $order->id]),
        'reference_id' => 'VNV-341' . $order->id,
        'customer_email' => $customerEmail,
    ], $_POST, ['source' => 'order_access_tip', 'order_id' => (int)$order->id]);

    if ($charge === false) {
        echo json_encode(['success' => false, 'error' => TranslationService::trans('planner_hub.payment_could_not_be_processed')]);
        exit;
    }
