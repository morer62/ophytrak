...
function resolveRequestedBusinessOwnerId(): int
{
    return (int)(
...
function userCanAccessBusiness(User $user, int $businessOwnerId): bool
{
    if ($businessOwnerId <= 0) {
        return true;
    }

    if ((int)$user->getOwner() === $businessOwnerId) {
        return true;
    }

    if ((int)$user->getLevel() === User::$CLIENT_USER_LEVEL) {
        return (new ClientsUsersRepository())->exists((int)$user->getId(), $businessOwnerId);
    }
...
$router->post(function () {
    ...
    try {
        ...
        $businessOwnerId = resolveRequestedBusinessOwnerId();
        if (!userCanAccessBusiness($userEntity, $businessOwnerId)) {
            LoginService::logout();
            return JsonResponse::createResponse([
                "success" => false,
                "message" => "This account is not associated with this mobile app."
            ], 403);
        }
        ...
        $expo_token = trim($_POST["expo_token"] ?? ($_POST["expo_push_token"] ?? ""));
