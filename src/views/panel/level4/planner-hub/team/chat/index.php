...
$storeTasksRepo = new StoreOrderTasksRepository();
...
function chat_panel_can_message_target(object $user, object $target, ClientsUsersRepository $clientsRepo, StoreOrderTasksRepository $storeTasksRepo): bool
{
    ...
    if ($level === 4) {
        ...
        return ($targetLevel !== 5 && ($targetId === $ownerId || $targetOwnerId === $ownerId))
            || (
                $targetLevel === 5
                && (
                    ($canChatWithClients && $clientsRepo->exists($targetId, $ownerId))
                    || $storeTasksRepo->assigneeCanChatWithClient($ownerId, (int)$user->getId(), $targetId)
                )
            );
    }
...
$router->get(function () use ($threadRepo, $messageRepo, $userRepo, $clientsRepo, $institutionProfileRepo, $storeTasksRepo): string {
    ...
    if ($toUserId > 0 && $threadId <= 0) {
        ...
        if (!$target || !chat_panel_can_message_target($user, $target, $clientsRepo, $storeTasksRepo)) {
    ...
    if ($threadId > 0) {
        $candidate = $threadRepo->getByIdForParticipant($threadId, (int)$user->getId());
...
$router->post(function () use ($threadRepo, $messageRepo, $userRepo, $clientsRepo, $storeTasksRepo): void {
    $user = LoginService::getSession();
    $to = (int)($_POST["to"] ?? 0);
