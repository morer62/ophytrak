...
$router->post(function () {
    ...
    if($action==='carrier_qr_claim'){
        if((string)($_POST['workflow_context']??'')==='physical_reception'){[$valid,$validationMessage,$candidate]=$repo->previewClaimByQr($ownerId,trim((string)($_POST['qr_token']??'')));if(!$valid||!$candidate||(string)$candidate->custody_status!=='PICKED_UP')$respond(false,$valid?'Este paquete no está pendiente de recepción física.':$validationMessage);}
