...
$router->get(function () {
    ...
    $isCarrierOwner=(int)$user->getLevel()===2;$carrierPackages=$isCarrierOrganization?$carrierRepo->getForCarrier($ownerId,$isCarrierOwner?null:(int)$user->getId()):[];$carrierCancelledPackages=$isCarrierOrganization?$carrierRepo->getCancelledReturnsForCarrier($ownerId,$isCarrierOwner?null:(int)$user->getId()):[];$globalStatusCounts=['collected'=>0,'warehouse'=>0,'route'=>0,'delivered'=>0,'incidents'=>0,'returns'=>count($carrierCancelledPackages)];$globalSellerSummary=[];$globalAgentSummary=[];foreach($carrierPackages as $package){$status=(string)$package->custody_status;if($status==='PICKED_UP')$globalStatusCounts['collected']++;elseif(in_array($status,['RECEIVED_AT_HUB','SORTED_AT_HUB'],true))$globalStatusCounts['warehouse']++;elseif($status==='OUT_FOR_DELIVERY')$globalStatusCounts['route']++;elseif($status==='DELIVERED')$globalStatusCounts['delivered']++;elseif(in_array($status,['CUSTOMER_ABSENT','CUSTOMER_REJECTED'],true))$globalStatusCounts['incidents']++;$seller=trim((string)($package->seller_name??''))?:'Vendedor #'.(int)$package->id_owner;if(!isset($globalSellerSummary[$seller]))$globalSellerSummary[$seller]=['name'=>$seller,'total'=>0,'open'=>0,'delivered'=>0];$globalSellerSummary[$seller]['total']++;if($status==='DELIVERED')$globalSellerSummary[$seller]['delivered']++;elseif(!in_array($status,['CLOSED','CANCELLED_RETURNED_TO_SELLER'],true))$globalSellerSummary[$seller]['open']++;$agent=trim((string)($package->agent_name??'').' '.(string)($package->agent_lastname??''))?:'Sin agente';if(!isset($globalAgentSummary[$agent]))$globalAgentSummary[$agent]=['name'=>$agent,'total'=>0,'route'=>0,'incidents'=>0];$globalAgentSummary[$agent]['total']++;if($status==='OUT_FOR_DELIVERY')$globalAgentSummary[$agent]['route']++;if(in_array($status,['CUSTOMER_ABSENT','CUSTOMER_REJECTED'],true))$globalAgentSummary[$agent]['incidents']++;}$globalSellerSummary=array_values($globalSellerSummary);$globalAgentSummary=array_values($globalAgentSummary);$today=(new \DateTimeImmutable('now',new \DateTimeZone('America/Sao_Paulo')))->format('Y-m-d');$historyFrom=preg_match('/^\d{4}-\d{2}-\d{2}$/',(string)($_GET['from']??''))?(string)$_GET['from']:(new \DateTimeImmutable($today))->modify('-30 days')->format('Y-m-d');$historyTo=preg_match('/^\d{4}-\d{2}-\d{2}$/',(string)($_GET['to']??''))?(string)$_GET['to']:$today;if($historyFrom>$historyTo)[$historyFrom,$historyTo]=[$historyTo,$historyFrom];$manifests=$isCarrierOrganization?($isCarrierOwner?$manifestRepo->getForCarrier($ownerId):$manifestRepo->getForDriver($ownerId,(int)$user->getId())):[];$activeManifest=null;$manifestItems=[];foreach($manifests as $candidate){if(in_array((string)$candidate->status,['DRAFT','GENERATED','IN_PROGRESS'],true)){$activeManifest=$candidate;$manifestItems=$manifestRepo->getItems((int)$candidate->id,$ownerId,$isCarrierOwner?0:(int)$user->getId());break;}}$manifestServiceSummary=[];$manifestRate=(float)($_ENV['OPHYTRACK_DEFAULT_DRIVER_PAYOUT_BRL']??0);foreach($manifestItems as $item){$source=strtoupper(trim((string)($item->order_source??'AVULSO')));$service=$source==='OPHYRA'?'TRACKEN WEB':str_replace('_',' ',$source);if(!isset($manifestServiceSummary[$service]))$manifestServiceSummary[$service]=['service'=>$service,'collections'=>0,'orders'=>0,'reships'=>0,'collection_value'=>0.0,'freight_value'=>0.0,'reship_value'=>0.0];$manifestServiceSummary[$service]['orders']++;$manifestServiceSummary[$service]['freight_value']+=$manifestRate;if(in_array((string)$item->status,['FAILED','RETURNED'],true)){$manifestServiceSummary[$service]['reships']++;$manifestServiceSummary[$service]['reship_value']+=$manifestRate;}}$manifestServiceSummary=array_values($manifestServiceSummary);
    return TemplateResponse::render(__DIR__ . '/index.twig', [
        ...
        'globalStatusCounts'=>$globalStatusCounts,'globalSellerSummary'=>$globalSellerSummary,'globalAgentSummary'=>$globalAgentSummary,
        ...
        'deliveryManifests'=>$manifests,'activeManifest'=>$activeManifest,'manifestItems'=>$manifestItems,'manifestServiceSummary'=>$manifestServiceSummary,'manifestDbReady'=>$manifestRepo->isReady(),
    ]);
});
...
$router->post(function () {
    ...
    if($action==='carrier_qr_claim'){
        $workflowContext=(string)($_POST['workflow_context']??'');if(in_array($workflowContext,['physical_reception','departure_manifest'],true)){[$valid,$validationMessage,$candidate]=$repo->previewClaimByQr($ownerId,trim((string)($_POST['qr_token']??'')));$allowed=$workflowContext==='physical_reception'?['PICKED_UP']:['RECEIVED_AT_HUB','SORTED_AT_HUB'];if(!$valid||!$candidate||!in_array((string)$candidate->custody_status,$allowed,true))$respond(false,$valid?($workflowContext==='physical_reception'?'Este paquete no está pendiente de recepción física.':'Este paquete no está listo para el manifiesto de salida.'):$validationMessage);}
