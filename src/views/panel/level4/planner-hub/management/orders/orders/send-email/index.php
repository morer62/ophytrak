...
$router->post(function () {
    ...
    try {
        // Get client (sin filtro de ownership para que nivel 4 encuentre cualquier cliente de la orden)
        $userRepo = new UserRepository();
        $client = $userRepo->getOneWithoutOwnership(['id' => (int)$clientId]);
        ...
        $ownerId = $order && isset($order['id_owner']) ? (int)$order['id_owner'] : null;
        
        if (!$ownerId && $user) {
            $ownerId = $user->getLevel() === 4 ? (int)$user->getOwner() : (int)$user->getId();
            if ($user->getLevel() === 4) {
