...
$router->post(function () use ($leadRepo, $user) {
    ...
    if (!$id || $name === "") {
        MessageUtil::setMessage(TranslationService::trans('planner_hub.name_required'));
    ...
    if ($user->getLevel() === 4) {
        ...
        if ($currentInstitutionId) {
            ...
            if ($institutionOwnerId) {
                ...
                if (!$lead) {
                    MessageUtil::setMessage(TranslationService::trans('planner_hub.lead_not_found_permission'));
                    LocationUtils::redirectInternal("panel/planner-hub/management/crm/lead");
                }
            } else {
    ...
    MessageUtil::setMessage(TranslationService::trans('planner_hub.lead_updated_successfully'));
    LocationUtils::redirectInternal("panel/planner-hub/management/crm/lead");
});
