...
$router->get(function () {
    ...
    $db->query('SELECT ts.* FROM ticket_sales ts INNER JOIN ticket_types tt ON tt.id=ts.id_ticket_type INNER JOIN venue_events_tickets vet ON vet.id=tt.id_venue_event_tickets INNER JOIN venue_events ve ON ve.id=vet.id_venue_event INNER JOIN venues v ON v.id=ve.venue_id WHERE ts.id=:ticket AND v.user_id=:owner LIMIT 1');
    $db->bind(':ticket',(int)$ticketId);$db->bind(':owner',(int)$user->getId());
    $ticket = $db->fetchOne();
