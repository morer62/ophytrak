...
$router->get(function () {
    ...
    try {
        ...
        error_log('Store product create categories failed: ' . $e->getMessage());
    }
    ...
    try {
        ...
        try {
            $valuesByAttribute = $attributeValuesRepo->getActiveGroupedByOwner($ownerId);
        } catch (\Throwable $e) {
            error_log('Store product create grouped attribute values failed: ' . $e->getMessage());
            $valuesByAttribute = [];
        }

        foreach ($attributes as $attribute) {
            $attribute->values = $valuesByAttribute[(int)$attribute->id] ?? [];
        }
    } catch (\Throwable $e) {
        error_log('Store product create attributes failed: ' . $e->getMessage());
        $attributes = [];
    }
...
$router->post(function () {
    ...
    if ($sku !== '' && $productsRepo->skuExists($sku, null, $ownerId)) {
    ...
    if ($productType === StoreProductsRepository::PRODUCT_TYPE_VARIABLE) {
        ...
        for ($i = 0; $i < $rowCount; $i++) {
            ...
            if ($variationSku !== '') {
                ...
                if ($productsRepo->skuExists($variationSku, null, $ownerId)) {
    ...
    $slug = $productsRepo->generateUniqueSlug($slugBase, null, $ownerId);
