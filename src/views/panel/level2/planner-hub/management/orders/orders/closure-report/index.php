...
$router->post(function () {
    ...
    $redirectToClosureReport = static function () use ($orderId): never {
        ...
        if (isset($_FILES["photos"]) && $_FILES["photos"]["error"][0] != UPLOAD_ERR_NO_FILE) {
            ...
            for ($i = 0; $i < $fileCount; $i++) {
                if ($files['error'][$i] !== UPLOAD_ERR_OK) {
                    $uploadErrors[] = "Skipped " . ($files['name'][$i] ?? 'file') . ": upload error.";
                    continue;
                }
                if (!in_array($files['type'][$i], $allowedTypes, true)) {
                    $uploadErrors[] = "Skipped " . ($files['name'][$i] ?? 'file') . ": invalid image type.";
                    continue;
                }
                if ($files['size'][$i] > $maxFileSize) {
                    $uploadErrors[] = "Skipped " . ($files['name'][$i] ?? 'file') . ": file is larger than 5MB.";
                    continue;
                }
                ...
                try {
                    ...
                    if ($photoUrl === '') {
                        throw new \RuntimeException("The upload did not return a public URL.");
                    }
                    ...
                    $uploadErrors[] = "Error uploading " . ($files['name'][$i] ?? 'photo') . ": " . $e->getMessage();
                }
            }
        }
        if ($uploadedCount > 0) {
            MessageUtil::setMessage($uploadedCount . " photo" . ($uploadedCount === 1 ? "" : "s") . " uploaded successfully" . ($uploadErrors ? ". Some files were skipped." : ""));
        } else {
            MessageUtil::setMessage($uploadErrors ? implode(" ", $uploadErrors) : "No photos were selected for upload.", "Error", "error");
        }
        $redirectToClosureReport();
        return;
    }
