...
$router->get(function () {
    ...
    $manualCharge = null;
    if ($id_order) {
        $manualCharge = (new AuthorizedManualChargeService())->getOrderEligibility((int)$id_order, $user);
    }
...
$router->post(function () {
    ...
    if ($action === 'authorized_manual_charge') {
        ...
        $methodId = (int)($_POST['saved_payment_method_id'] ?? 0);
        $amount = (float)($_POST['manual_charge_amount'] ?? 0);
        $result = (new AuthorizedManualChargeService())->chargeOrderBalance($id_order, $methodId, $amount, LoginService::getSession());
        MessageUtil::setMessage($result['message'] ?? ($result['success'] ? 'Charge processed.' : 'Charge failed.'), $result['success'] ? 'Success' : 'Error', $result['success'] ? 'success' : 'error');
    ...
    if (!$account || !$account->square_account_id) {
        MessageUtil::setMessage("❌ " . TranslationService::trans('planner_hub.square_account_not_found'));
        LocationUtils::reload();
    }
    ...
    if (!$payment) {
        MessageUtil::setMessage("❌ " . TranslationService::trans('planner_hub.payment_not_found'));
        LocationUtils::reload();
    }
    ...
    if (!$refund) {
        MessageUtil::setMessage("❌ " . TranslationService::trans('planner_hub.error_refunding_charge'));
        LocationUtils::reload();
    }
    ...
    if ($refundAmount == $payment->amount) {
        MessageUtil::setMessage("✅ " . TranslationService::trans('planner_hub.full_refund_processed'));
    } else if ($total_refunded == $payment->amount){
        MessageUtil::setMessage("✅ " . TranslationService::trans('planner_hub.full_refund_processed'));
    } else {
        MessageUtil::setMessage("✅ " . TranslationService::trans('planner_hub.partial_refund_processed'));
    }
