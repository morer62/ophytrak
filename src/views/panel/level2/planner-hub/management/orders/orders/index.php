...
$router->get(callback: function () {
    ...
    // Para nivel 4: obtener todos los clientes de las órdenes mostradas, incluso si tienen diferente id_owner
    // Esto asegura que siempre se muestre la información del cliente correcta
    if ($user->getLevel() === 4) {
        $clientIds = [];
        // Obtener IDs de clientes de órdenes y estimaciones que se están mostrando
        foreach ($orders as $order) {
            if ($order->id_client && !in_array($order->id_client, $clientIds)) {
                $clientIds[] = $order->id_client;
            }
        }
        foreach ($estimates as $estimate) {
            if ($estimate->id_client && !in_array($estimate->id_client, $clientIds)) {
                $clientIds[] = $estimate->id_client;
            }
        }
        
        // Obtener información de todos los clientes de las órdenes
        $clientsFromOrders = [];
        foreach ($clientIds as $clientId) {
            $clientInfo = $clientRepo->getOneWithoutOwnership(["id" => $clientId, "level" => 5]);
            if ($clientInfo) {
                $clientsFromOrders[] = $clientInfo;
            }
        }
        
        // Combinar con los clientes existentes (evitar duplicados)
        $existingClientIds = array_column($clients, 'id');
        foreach ($clientsFromOrders as $clientFromOrder) {
            if (!in_array($clientFromOrder->id, $existingClientIds)) {
                $clients[] = $clientFromOrder;
            }
        }
    }
...
$router->post(function () {
    ...
    if (isset($_POST['action']) && $_POST['action'] === 'add_manual_advance') {
        ...
        $paymentMethod = trim((string)($_POST['advance_payment_method'] ?? 'manual'));
        $paymentReference = trim((string)($_POST['advance_reference'] ?? ''));
        $proofUrl = '';
        ...
        $sessionOwnerId = (int)($session->getOwner() ?: $session->getId());
        if ((int)($order->id_owner ?? 0) !== $sessionOwnerId) {
            MessageUtil::setMessage('Order not found in this workspace.', 'Error', 'error'); LocationUtils::reload();
        }
        ...
        if (FileUtils::hasFile($_FILES, 'advance_proof')) {
            $file=$_FILES['advance_proof'];
            if (!in_array((string)($file['type']??''),['image/jpeg','image/png','image/webp','application/pdf'],true)||(int)($file['size']??0)>5*1024*1024) { MessageUtil::setMessage('Payment proof must be a JPG, PNG, WEBP or PDF up to 5 MB.','Error','error');LocationUtils::reload(); }
            $proofUrl=FileUtils::saveFile($file,'service-manual-advances');
        }

        try {
            $db->query("INSERT INTO orders_advances (id_order, is_suborder, id_suborder, amount, payment_method, payment_reference, proof_url, total_before, total_after, note, created_by, created_at) VALUES (:order_id, 0, NULL, :amount, :method, :reference, :proof, :before, :after, :note, :created_by, NOW())");
            $db->bind(":order_id", $orderId);
            $db->bind(":amount", $amount);
            $db->bind(":method",$paymentMethod);$db->bind(":reference",$paymentReference?:null);$db->bind(":proof",$proofUrl?:null);$db->bind(":created_by",(int)$session->getId());
