...
$router->get(function () {
    ...
    $managementContext = (new \App\Services\ManagementOwnerContextService())->resolve($session, $userInstitutionService, $institutionRepo);
    ...
    $clientsRepoForAccess = new ClientsUsersRepository();
    $isAssociatedClient = (int)$user->level === 5 && $clientsRepoForAccess->exists((int)$user->id, (int)$currentOwnerId);

    if ($isAssociatedClient && (int)$user->password_updated === 0) {
        $canEdit = true;
    } elseif ($user->id_owner == $currentOwnerId) {
        $canAccess = true;
...
$router->post(function () {
    ...
    if ($user->level == 5 && $user->password_updated == 1) {
        // Validated clients own their global identity. A business can only
        // manage its association and cannot overwrite the personal profile.
