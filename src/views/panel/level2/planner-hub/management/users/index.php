...
$router->post(function () {
    ...
    if (isset($_POST["delete_client"])) {
        ...
        if ($targetClient) {
            $clientsUsersRepo->deleteRelation($clientId, $currentOwnerId);
            MessageUtil::setMessage("Client unlinked from this business. The account and historical data were preserved.");
        } else {
        ...
        if ($currentInstitutionId) {
            ...
            if ($targetUser) {
                ...
                if ((int) $targetUser->level === 4) {
                    ...
                    $clientsUsersRepo->deleteRelation($id, $currentOwnerId);
                    MessageUtil::setMessage("Client unlinked from this business. The account and historical data were preserved.");
                } else {
                    if ($userInstitutionRecord) {
                        $userInstitutionsRepo->removeUserFromInstitution($id, $currentInstitutionId);
