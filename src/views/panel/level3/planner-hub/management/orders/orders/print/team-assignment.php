...
foreach ($servicesAssigned as $assigned) {
    ...
    $menuSelections = $menuSelectionsRepo->getByService($orderId, null, $assigned->id_service, 'main_order');
...
foreach ($suborders as $suborder) {
    ...
    foreach ($suborderServicesAssigned as $assigned) {
        ...
        $menuSelections = $menuSelectionsRepo->getByService($orderId, (int)$suborder->id, $assigned->id_service, 'suborder');
...
foreach ($services as $assigned) {
    ...
    $menuSelections = $assigned["menu_selections"] ?? [];
    ...
    if ($noteEntry) {
        ...
        $start = (isset($noteEntry->start_time) && $noteEntry->start_time) ? date("g:i A", strtotime($noteEntry->start_time)) : "--";

        $pdf->SetTextColor(255, 0, 0);
        $pdf->SetFont('Arial', 'B', 11);
        $pdf->Cell(0, 8, "Installation Time: $install     Execution: $execution     Breakdown: $breakdown     Start Time: $start", 0, 1);
    ...
    if (!empty($menuSelections)) {
        ...
        $pdf->SetFillColor(0, 0, 0);
        $pdf->SetTextColor(255, 255, 255);
        $pdf->Cell(0, 8, "Menu Selection", 1, 1, 'L', true);
        $pdf->SetTextColor(0, 0, 0);

        // Group items by category for cleaner PDF output
        $groupedMenu = [];
        foreach ($menuSelections as $sel) {
            $category = $sel->category_name ?? '-';
            $item = $sel->item_name ?? '-';
            if (!empty($sel->price_info)) {
                $item .= " ({$sel->price_info})";
            }
        ...
        foreach ($groupedMenu as $category => $items) {
            ...
            $pdf->SetFillColor(230, 230, 230);
            $pdf->Cell(0, 8, utf8_decode($category), 1, 1, 'L', true);

            $pdf->SetFont('Arial', '', 9);
            foreach ($items as $itemText) {
                $pdf->Cell(0, 7, '- ' . utf8_decode($itemText), 1, 1);
            }
        }
