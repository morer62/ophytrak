...
$router->get(function () {
    ...
    $id = $_GET["id"] ?? null;
    if (!$id) {
        MessageUtil::setMessage(TranslationService::trans('planner_hub.invalid_user_id'));
...
if ($action === "addManualHour") {
    ...
    if (!$userId || !$startInput || !$endInput) {
        MessageUtil::setMessage(TranslationService::trans('planner_hub.provide_start_end_user'));
    ...
    if (strtotime($endTime) <= strtotime($startTime)) {
        MessageUtil::setMessage(TranslationService::trans('planner_hub.end_time_must_later'));
        LocationUtils::reload();
    }
    ...
    if ($created) {
        MessageUtil::setMessage(TranslationService::trans('planner_hub.manual_hours_added'));
    } else {
        MessageUtil::setMessage(TranslationService::trans('planner_hub.unable_add_manual_hours'), "Error", "error");
    }
...
if ($action === "deleteManualHour") {
    ...
    TranslationService::detectLocale();
    MessageUtil::setMessage(TranslationService::trans('planner_hub.manual_hour_removed'));
    ...
    if ($action === "editStatDate" || $action === "editEndDate") {
        ...
        if (!$id || !$value || !$timezone) {
            MessageUtil::setMessage(TranslationService::trans('planner_hub.invalid_record_id_date'));
            LocationUtils::reload();
        }
        ...
        MessageUtil::setMessage(TranslationService::trans('planner_hub.time_updated_successfully'));
        LocationUtils::reload();
    }
    ...
    if ($action === "editNotes") {
        ...
        if (!$id) {
            MessageUtil::setMessage(TranslationService::trans('planner_hub.invalid_record_id'));
            LocationUtils::reload();
        }

        $repoHours->update(["notes" => $notes], ["id" => $id]);
        MessageUtil::setMessage(TranslationService::trans('planner_hub.notes_updated_successfully'));
        LocationUtils::reload();
    }
    ...
    if (!$userId || empty($ids)) {
        MessageUtil::setMessage(TranslationService::trans('planner_hub.invalid_input_no_hours'));
        LocationUtils::reload();
    }
    ...
    if (empty($validIds)) {
        MessageUtil::setMessage(TranslationService::trans('planner_hub.no_valid_sessions_selected'));
        LocationUtils::reload();
    }
    ...
    try {
        $emailService = new EmailService($currentOwnerId);
   ...
   MessageUtil::setMessage(TranslationService::trans('planner_hub.hours_paid_successfully'));
   LocationUtils::redirectInternal("panel/planner-hub/management/payroll/pending");
});
