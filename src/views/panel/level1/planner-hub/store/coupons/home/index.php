...
$router->get(function () {
    ...
    $coupons = $ownerId > 0 ? $couponsRepo->getAllByOwner($ownerId, 500) : [];
    ...
    $clients = $userRepo->getActiveClientsByOwner($ownerId);
...
$router->post(function () {
    ...
    if ($action === 'create' || $action === 'update') {
        ...
        if ($action === 'create') {
            ...
            if ($ok && $scope === StoreCouponsRepository::SCOPE_CUSTOMER && $assignedCustomerEmail !== '') {
                ...
                $user = $userRepo->getActiveClientByOwnerAndEmail($ownerId, $assignedCustomerEmail);
                $customersRepo->add([
                    'id_coupon' => $couponId,
    ...
    if ($action === 'assign_customer') {
        ...
        $user = $userRepo->getActiveClientByOwnerAndEmail($ownerId, $email);
