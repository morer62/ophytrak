...
$router->get(function () {
    ...
    $subscriptions = $ownerId > 0 ? $repo->getScopedByOwner($ownerId, 300) : [];
...
$router->post(function () {
    ...
    $user = $userId > 0 ? $userRepo->getActiveClientByOwnerAndId($ownerId, $userId) : null;
    if (!$user && !empty($subscription->email)) {
        $user = $userRepo->getActiveClientByOwnerAndEmail($ownerId, (string)$subscription->email);
        if ($user) {
            $userId = (int)$user->id;
    ...
    if (!$user || $userId <= 0) {
        MessageUtil::setMessage('Renewal failed: the customer is not assigned to this business workspace.');
        LocationUtils::reload();
    }
