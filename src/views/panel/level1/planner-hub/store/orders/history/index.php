...
$router->get(function () {
    ...
    $orders = $ownerId > 0 ? $ordersRepo->getHistoryByOwner($ownerId, $fromDate->setTime(0,0)->format('Y-m-d H:i:s'), $toDate->setTime(23,59,59)->format('Y-m-d H:i:s'), 500) : [];
...
$router->post(function () {
    ...
    if ($ok && $status === StoreOrdersRepository::STATUS_RETURNED) {
        $ordersRepo->markAsRefunded($orderId);
    }
