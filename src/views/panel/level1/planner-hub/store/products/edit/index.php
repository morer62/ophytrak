...
$router->get(function () {
    ...
    $product = $ownerId > 0 ? $productsRepo->getFullProductDetails($id, $ownerId) : null;
    ...
    $variations = $variationsRepo->getDetailedByProduct($id, $ownerId);
...
$router->post(function () {
    ...
    $existingProduct = $ownerId > 0 ? $productsRepo->getOneByOwner(['id' => $id], $ownerId) : null;
    ...
    if ($sku !== '' && $productsRepo->skuExists($sku, $id, $ownerId)) {
    ...
    if ($productType === StoreProductsRepository::PRODUCT_TYPE_VARIABLE) {
        ...
        for ($i = 0; $i < $rowCount; $i++) {
            ...
            if ($variationSku !== '') {
                ...
                if ($productsRepo->skuExists($variationSku, $id, $ownerId)) {
    ...
    $slug = $productsRepo->generateUniqueSlug($slugBase, $id, $ownerId);
