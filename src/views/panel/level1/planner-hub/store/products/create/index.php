...
$router->get(function () {
    ...
    return TemplateResponse::render(__DIR__ . "/index.twig", [
        "categories" => $ownerId > 0 ? $categoriesRepo->getActive($ownerId) : [],
...
$router->post(function () {
    ...
    if ($name === '') {
        ...
        LocationUtils::redirectInternal("panel/planner-hub/store/products/create" . ($operationQuery ? '?' . $operationQuery : ''));
    }

    if ($ownerId <= 0) {
        MessageUtil::setMessage("Select a valid central operation before creating products.");
    ...
    if ($productType === StoreProductsRepository::PRODUCT_TYPE_VARIABLE) {
        ...
        for ($i = 0; $i < $rowCount; $i++) {
            ...
            if ($variationSku !== '' && $productsRepo->skuExists($variationSku, null, $ownerId)) {
