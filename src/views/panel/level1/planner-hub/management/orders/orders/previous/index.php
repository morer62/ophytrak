...
$router->get(callback: function () {
    ...
    // Determinar el owner ID según el nivel del usuario
    $ownerId = null;
    if ($user->getLevel() === 4) {
        ...
        if (!$ownerId) {
            // Si no hay institución activa, no mostrar órdenes
            $orders = [];
            $totalOrders = 0;
            $totalPages = 0;
        } else {
            // Obtener todas las órdenes del owner y filtrar por fecha pasada
            $allOrders = $repo->getAllByInstitutionOwner($ownerId, 0);
            
            // Filtrar por fecha pasada
            $today = date('Y-m-d');
            $filteredOrders = array_filter($allOrders, function($order) use ($today, $startDate, $endDate, $search) {
                $orderDate = is_object($order) ? $order->event_date : $order['event_date'];
                if ($orderDate >= $today) {
                    return false;
                }
                ...
                if ($search) {
                    ...
                    $address = is_object($order) ? strtolower($order->address ?? '') : strtolower($order['address'] ?? '');
                    if (strpos($address, $searchLower) === false) {
                        return false;
                    }
                }
            ...
            usort($filteredOrders, function($a, $b) {
                $dateA = is_object($a) ? $a->event_date : $a['event_date'];
                $dateB = is_object($b) ? $b->event_date : $b['event_date'];
        ...
        // Para niveles 1, 2, 3 usar el ID del usuario directamente
...
$router->post(function () {
    ...
    if (isset($_POST['archive_order_id'])) {
        ...
        // Verificar que la orden existe y pertenece al usuario/owner
        ...
        if ($session->getLevel() === 4) {
            ...
            if ($currentInstitutionId) {
                $institutionRepo = new \App\Repositories\InstitutionProfileRepository();
                $institution = $institutionRepo->getById($currentInstitutionId);
                if ($institution && $institution->id_owner && $order->id_owner == $institution->id_owner) {
                    $hasAccess = true;
                }
            }
        } else {
