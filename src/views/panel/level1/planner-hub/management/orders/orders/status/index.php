...
$router->post(function () {
    ...
    $paidAtManual = null;
    if (!empty($_POST['paid_at_manual'])) {
        $parsed = \DateTime::createFromFormat('Y-m-d\TH:i', $_POST['paid_at_manual']);
        if ($parsed) {
            $paidAtManual = $parsed->format('Y-m-d H:i:s');
        }
    }
    if (!$paidAtManual) {
        $paidAtManual = date('Y-m-d H:i:s');
    }
