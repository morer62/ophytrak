...
$router->get(function () {
    ...
    foreach ($servicesAssigned as $assigned) {
        ...
        foreach ($tasks as $task) {
            ...
            // Obtener fotos de la tarea si está asignada
            if ($assignedTask?->id) {
                $task->photos = $photosRepo->getByTask($assignedTask->id);
            } else {
                $task->photos = [];
            }
        }
    ...
    // Obtener fotos para tareas manuales
...
$router->post(function () {
    ...
    if ($formType === "manual_task_add") {
        if (!empty($_POST["description"]) && !empty($_POST["id_user"])) {
            ...
            NotificationService::sendToUsers(
                [$_POST["id_user"]],
                '✅ ' . TranslationService::trans('planner_hub.new_task_assigned'),
                TranslationService::trans('planner_hub.you_have_been_assigned') . ' 1 ' . TranslationService::trans('planner_hub.new_task') . ' ' . TranslationService::trans('planner_hub.for_order') . ' VNV 341' . $orderId . '.'
            );
    ...
    if ($formType === "auto_tasks") {
        ...
        foreach ($assignedCountPerUser as $uid => $count) {
            $taskWord = $count > 1 ? TranslationService::trans('planner_hub.new_tasks') : TranslationService::trans('planner_hub.new_task');
            NotificationService::sendToUsers(
                [$uid],
                '✅ ' . TranslationService::trans('planner_hub.new_tasks_assigned'),
                TranslationService::trans('planner_hub.you_have_been_assigned') . ' ' . $count . ' ' . $taskWord . ' ' . TranslationService::trans('planner_hub.for_order') . ' VNV 341' . $orderId . '.'
            );
        }
