...
$router->post(function () {
    ...
    $decimalOrZero = static function ($value): float {
        if ($value === null) {
            return 0.0;
        }
    ...
    $discountInputValue = $decimalOrZero($_POST["discount_value"] ?? 0);
    $taxPercentage = $decimalOrZero($_POST["tax_percentage"] ?? 0);

    // Calcular el descuento real si no es edición limitada
    $actual_discount_value = $discountInputValue;
    if (!$isLimited) {
