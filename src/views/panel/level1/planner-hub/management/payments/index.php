...
$router->get(function () {
    ...
    $paymentBreakdown = [
        'order_payments' => 0.0,
        'suborder_payments' => 0.0,
        'advances' => 0.0,
    ];
    foreach ($collectedPayments as $p) {
        $net = (float)$p->amount - (float)($p->refunded_amount ?? 0);
        $totalCollected += $net;
        if (in_array(($p->entry_type ?? ''), ['advance', 'suborder_advance'], true)) {
            $paymentBreakdown['advances'] += $net;
        } elseif (($p->payment_kind ?? '') === 'suborder' || ($p->payment_scope ?? '') === 'suborder') {
            $paymentBreakdown['suborder_payments'] += $net;
        } else {
            $paymentBreakdown['order_payments'] += $net;
        }
    }
