...
$router->post(function () {
    ...
    if (isset($_POST['new_role']) && !empty($_POST['new_role'])) {
        ...
        if (count($custom_roles) >= 6) {
            MessageUtil::setMessage(TranslationService::trans('planner_hub.max_custom_roles_reached'));
    ...
    if (isset($_POST['role_id']) && isset($_POST['permissions'])) {
        ...
        if ($role && $role->id_owner == 0) {
            MessageUtil::setMessage(TranslationService::trans('planner_hub.system_roles_cannot_modify'));
            LocationUtils::reload();
        }
    ...
    if (isset($_POST['delete_role_id'])) {
        ...
        if ($role && $role->id_owner == 0) {
            MessageUtil::setMessage(TranslationService::trans('planner_hub.system_roles_cannot_delete'));
            LocationUtils::reload();
        }
