...
$router->get(function () {
    ...
    if (!is_numeric($serviceId)) {
        MessageUtil::setMessage(TranslationService::trans('planner_hub.invalid_service_id'), 'error');
    ...
    if (!$service) {
        MessageUtil::setMessage(TranslationService::trans('planner_hub.service_not_found'), 'error');
...
$router->post(function () {
    ...
    if (isset($_POST['action']) && $_POST['action'] === 'update_status' && 
        ...
        if (!$request) {
            MessageUtil::setMessage(TranslationService::trans('planner_hub.request_not_found'), 'error');
        } else {
            $requestRepo->update(
                ['status' => $status],
                ['id' => $requestId]
            );
            MessageUtil::setMessage(TranslationService::trans('planner_hub.status_updated_successfully'), 'success');
        }
...
try {
    ...
    TranslationService::detectLocale();
    MessageUtil::setMessage(str_replace('{error}', $e->getMessage(), TranslationService::trans('planner_hub.error_occurred')), 'error');
    LocationUtils::redirectTo($_ENV['APP_URL'] . "/panel/service/home");
}