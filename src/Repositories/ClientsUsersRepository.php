    ...
    public function getAssociatedCompaniesForClient(int $clientId): array
    {
        $this->db->query("
            SELECT
                ...
                COALESCE(ip.company_name, CONCAT(u.name, ' ', u.lastname)) AS company_name,
                ip.logo_path,
                ip.email AS business_email,
                ip.phone AS business_phone,
                ip.city,
                ip.state,
            ...
            LEFT JOIN users u ON u.id = cu.id_owner_asociated
            WHERE cu.client_id = :client_id
            ORDER BY ip.company_name ASC, cu.created_at DESC
        ");
        $this->db->bind(":client_id", $clientId);
