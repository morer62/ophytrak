...
{
    private ?bool $hasAssignmentAuditColumns = null;
    private ?bool $hasCancellationColumns = null;
    ...
    {
        $auditSelect = $this->supportsAssignmentAudit()
            ? 't.assigned_by, t.assigned_at, assigner.name AS assigned_by_name, assigner.lastname AS assigned_by_lastname'
            : 'NULL AS assigned_by, t.created_at AS assigned_at, NULL AS assigned_by_name, NULL AS assigned_by_lastname';
        $auditJoin = $this->hasAssignmentAuditColumns ? 'LEFT JOIN users assigner ON assigner.id = t.assigned_by' : '';
        $this->db->query("SELECT t.*, u.name, u.lastname, u.email, {$auditSelect}
            FROM {$this->table} t
    ...
    {
        $cancellationSelect = $this->supportsCancellationColumns()
            ? 'o.cancellation_status, o.cancellation_reason,'
            : "'NONE' AS cancellation_status, NULL AS cancellation_reason,";
        $this->db->query("
            SELECT t.*, o.guest_name, o.guest_email, o.guest_phone, o.id_user AS client_user_id, o.status AS order_status,
                   {$cancellationSelect}
                   o.payment_status, o.shipping_address_1, o.shipping_address_2, o.shipping_city,
                   o.shipping_state, o.shipping_zip, o.shipping_country, o.shipping_instructions,
                   o.created_at AS order_created_at, o.public_token, p.package_code,
            ...
            LEFT JOIN store_packages p ON p.id_owner=t.id_owner AND p.id_store_order=t.id_store_order AND p.package_sequence=1
            WHERE t.id_owner = :owner AND t.id_user = :user
              AND (t.task_type <> 'DELIVERY' OR COALESCE(p.logistics_mode, 'INTERNAL') = 'INTERNAL')
    ...
    public function createTask(int $ownerId, int $orderId, ?int $userId, string $type, string $title, ?string $instructions = null, bool $requiresLocation = false, bool $allowAssigneeComplete = true, ?int $assignedBy = null): bool
    {
        if (!in_array($type, $this->types(), true) || trim($title) === '') {
            return false;
        }

        $auditColumns = $this->supportsAssignmentAudit() ? ', assigned_by, assigned_at' : '';
        $auditValues = $this->hasAssignmentAuditColumns ? ', :assigned_by, NOW()' : '';
        $this->db->query("INSERT INTO {$this->table}
                (id_owner, id_store_order, id_user, task_type, title, instructions, status,
                 requires_location, allow_assignee_complete{$auditColumns}, created_at, updated_at)
            VALUES
                (:owner, :order, :user, :type, :title, :instructions, :status,
                 :requires_location, :allow_complete{$auditValues}, NOW(), NOW())");
        $this->db->bind(':owner', $ownerId);
        $this->db->bind(':order', $orderId);
        $this->db->bind(':user', $userId);
        ...
        if ($this->hasAssignmentAuditColumns) {
    ...
    private function supportsCancellationColumns(): bool
    {
        if ($this->hasCancellationColumns !== null) return $this->hasCancellationColumns;
        try {
            $this->db->query("SHOW COLUMNS FROM store_orders LIKE 'cancellation_status'");
            return $this->hasCancellationColumns = (bool)$this->db->fetchOne();
        } catch (\Throwable $e) {
            return $this->hasCancellationColumns = false;
        }
    }

    private function supportsAssignmentAudit(): bool
    {
        if ($this->hasAssignmentAuditColumns !== null) {
            return $this->hasAssignmentAuditColumns;
        }

        try {
            $this->db->query("SHOW COLUMNS FROM {$this->table} LIKE 'assigned_by'");
            $this->hasAssignmentAuditColumns = (bool)$this->db->fetchOne();
        } catch (\Throwable $e) {
            $this->hasAssignmentAuditColumns = false;
        }

        return $this->hasAssignmentAuditColumns;
    }

    public function ensureAssignment(int $ownerId, int $orderId, int $userId, string $type, string $title, bool $requiresLocation, ?int $assignedBy = null): bool
    {
        $this->db->query("
        ...
        return $this->createTask($ownerId, $orderId, $userId, $type, $title, null, $requiresLocation, true, $assignedBy);
    }

    public function replaceAssignmentByType(int $ownerId, int $orderId, ?int $userId, string $type, string $title, bool $requiresLocation, ?int $assignedBy = null): bool
    {
        ...
        return $this->ensureAssignment($ownerId, $orderId, $userId, $type, $title, $requiresLocation, $assignedBy);
    }
