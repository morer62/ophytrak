...
class CarrierPackageRepository extends StoreRepository
{
    ...
    public function claimByQr(int $carrierOwner,int $userId,string $token,?string $photoUrl=null,?float $lat=null,?float $lng=null,string $method='SECURE_QR'): array
    {
        ...
        $this->notify((int)$package->id_owner,'La transportadora recolectó el paquete '.$package->package_code.' mediante escaneo QR seguro.','panel/planner-hub/store/orders/home?package='.urlencode((string)$package->package_code));
    ...
    public function getForCarrier(int $carrierOwner,?int $userId=null): array
    {
        ...
        $this->db->query("SELECT DISTINCT p.*,o.guest_name,o.guest_email,o.guest_phone,o.shipping_address_1,o.shipping_address_2,o.shipping_city,o.shipping_state,o.shipping_zip,o.shipping_country,o.shipping_instructions,o.status AS order_status,ip.company_name AS seller_name,a.assignment_role,a.status AS assignment_status,u.name AS agent_name,u.lastname AS agent_lastname FROM store_packages p JOIN store_orders o ON o.id=p.id_store_order AND o.id_owner=p.id_owner LEFT JOIN institution_profile ip ON ip.id_owner=p.id_owner LEFT JOIN store_package_carrier_assignments a ON a.id_store_package=p.id AND a.carrier_owner_id=:carrier AND a.status<>'CANCELLED' LEFT JOIN users u ON u.id=p.current_custodian_user_id WHERE p.current_custodian_owner_id=:carrier {$userSql} ORDER BY p.last_event_at DESC");$this->db->bind(':carrier',$carrierOwner);if($userId)$this->db->bind(':user',$userId);return$this->db->fetchAll();
    }
    ...
    public function advance(int $packageId,int $carrierOwner,int $userId,string $action,string $notes='',?string $photo=null,array $meta=[]): array
    {
        ...
        [$status,$location]=$map[$action];if(in_array($action,['delivered','customer_absent','customer_rejected','delivery_cancelled'],true)&&!$photo)return[false,'Final delivery evidence photo is required.'];
