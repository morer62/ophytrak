...
class UserRepository extends BaseRepository
{
    ...
    /**
     * Active customers that belong directly to one business workspace.
     * Store & Logistics must never use the global client list.
     */
    public function getActiveClientsByOwner(int $ownerId): array
    {
        if ($ownerId <= 0) {
            return [];
        }
        $this->db->query("SELECT id, id_owner, level, is_active, name, lastname, email, phone
            FROM {$this->table}
            WHERE id_owner = :owner_id AND level = 5 AND is_active = 1
            ORDER BY name ASC, lastname ASC, email ASC");
        $this->db->bind(':owner_id', $ownerId);
        return array_values(array_filter(array_map(
            fn(object $client) => $this->sanitizeClientIdentity($client),
            $this->db->fetchAll()
        )));
    }

    public function getActiveClientByOwnerAndId(int $ownerId, int $clientId): ?object
    {
        if ($ownerId <= 0 || $clientId <= 0) {
            return null;
        }
        $this->db->query("SELECT id, id_owner, level, is_active, name, lastname, email, phone
            FROM {$this->table}
            WHERE id = :id AND id_owner = :owner_id AND level = 5 AND is_active = 1
            LIMIT 1");
        $this->db->bind(':id', $clientId);
        $this->db->bind(':owner_id', $ownerId);
        $client = $this->db->fetchOne();
        return $client ? $this->sanitizeClientIdentity($client) : null;
    }

    public function getActiveClientByOwnerAndEmail(int $ownerId, string $email): ?object
    {
    ...
    private function sanitizeClientIdentity(object $client): ?object
    {
        $email = strtolower(trim((string)($client->email ?? '')));
        // A credential sentence stored in an identity column is not an email and
        // must never be propagated to a form or template.
        ...
        if (preg_match('/(?:contrase(?:ña|na)?|password|passwd|senha|passcode|usuario\s*:)/iu', $identityText)) {
     ...
     */
    public function updateUserLanguages(int $userId, string $uiLanguage, string $systemLanguage): void
    {
        $this->db->query("UPDATE users SET ui_language = :ui_language, system_language = :system_language WHERE id = :id");
        $this->db->bind(":ui_language", $uiLanguage);
        $this->db->bind(":system_language", $systemLanguage);
    ...
    public function updateMembershipAndRegisterPayment(
        int $userId,
        string $newDueDate,
        float $amount,
    ...
    ): int
    {
        ...
        return $this->saveMembershipPaymentToAll($userId, floatval($amount), $newDueDate, 1, $reference, $transactionId, $billingMetadata);
    }

    private function saveMembershipPaymentToAll(
        int $userId,
        float $amount,
        string $renewalDate,
        int $planId = 1,
    ...
    ): int
    {
        ...
        $transactionId = $transactionId ?: "membership_{$userId}_" . time();

        $db = new Connection();

        if ($transactionId !== '') {
            $db->query("SELECT id FROM payments_all WHERE billing_transaction_id = :transaction_id LIMIT 1");
        ...
        $db->query("INSERT INTO payments_all 
            ...
            VALUES (:user_id, 'Membership', :concept_id, :plan_id, :payment_date, :renewal, :total, 'ACTIVE', :ref,
             :base_amount, :base_currency, :display_amount, :display_currency, :payment_amount, :payment_currency,
             :exchange_rate, :provider_type, :payment_method, :billing_transaction_id, :stripe_event_id,
             :stripe_invoice_id, :stripe_payment_intent_id, NULL)");
        ...
        $db->bind(":ref", $reference ?: "Membership payment");
        $db->bind(":base_amount", $billingMetadata['base_amount'] ?? $amount);
        $db->bind(":base_currency", $billingMetadata['base_currency'] ?? 'USD');
        $db->bind(":display_amount", $billingMetadata['display_amount'] ?? $amount);
        $db->bind(":display_currency", $billingMetadata['display_currency'] ?? 'USD');
        $db->bind(":payment_amount", $billingMetadata['payment_amount'] ?? $amount);
        $db->bind(":payment_currency", $billingMetadata['payment_currency'] ?? 'USD');
        $db->bind(":exchange_rate", $billingMetadata['exchange_rate'] ?? 1);
        $db->bind(":provider_type", $billingMetadata['provider_type'] ?? 'stripe');
        $db->bind(":payment_method", $billingMetadata['payment_method'] ?? 'saved_card');
        $db->bind(":billing_transaction_id", $transactionId);
        $db->bind(":stripe_event_id", $billingMetadata['stripe_event_id'] ?? null);
        $db->bind(":stripe_invoice_id", $billingMetadata['stripe_invoice_id'] ?? null);
        $db->bind(":stripe_payment_intent_id", $billingMetadata['stripe_payment_intent_id'] ?? null);
        ...
        try {
            ...
            $affiliateService->createCommission($userId, $transactionType, $amount, $transactionId, null, $paymentId ?: null);
        } catch (\Exception $e) {
   ...
   public function delete_level_5 ( $where): void
    ...
    public function getAssociatedClientsCount(int $ownerId): int
    {
        $sql = "
