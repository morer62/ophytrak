...
class OphytrackDriverPayoutRepository extends StoreRepository
{
    ...
    public function adjustmentsForDriver(int $user,int $carrierOwner,string $from,string $to):array
    {
        $this->db->query("SELECT COUNT(*) total FROM information_schema.tables WHERE table_schema=DATABASE() AND table_name='ophytrack_driver_earnings_adjustments'");if((int)($this->db->fetchOne()->total??0)!==1)return[];
        $this->db->query("SELECT a.*,CONVERT_TZ(a.effective_at,'+00:00','-03:00') effective_local,u.name created_by_name,u.lastname created_by_lastname FROM ophytrack_driver_earnings_adjustments a LEFT JOIN users u ON u.id=a.created_by_user_id WHERE a.delivery_user_id=:user AND a.carrier_owner_id=:carrier AND DATE(CONVERT_TZ(a.effective_at,'+00:00','-03:00')) BETWEEN :from_date AND :to_date ORDER BY a.effective_at DESC,a.id DESC");$this->db->bind(':user',$user);$this->db->bind(':carrier',$carrierOwner);$this->db->bind(':from_date',$from);$this->db->bind(':to_date',$to);return$this->db->fetchAll();
    }
