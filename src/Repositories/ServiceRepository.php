...
public function getLastApproved(int $limit = 5): array
{
    ...
    /**
     * Búsqueda global por categorías (sin filtrar por distancia/país)
     * Sistema global: muestra servicios de cualquier ubicación
     */
    public function searchByCategoriesGlobal(array $categories, int $limit = 0): array
    {
        $hasCategories = count($categories) > 0;
        if ($hasCategories) {
            $placeholders = implode(',', array_map(function ($i) { return ":c$i"; }, array_keys($categories)));
            $whereCategory = "AND s.category_id IN ($placeholders)";
        } else {
            $whereCategory = "";
        }
    ...
    /**
     * Obtener todos los servicios aprobados globalmente (sin filtro de ubicación)
     */
    public function getAllApprovedGlobal(int $limit = 0): array
    {
        $limitSQL = $limit > 0 ? "LIMIT :limit" : "";
