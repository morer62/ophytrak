...
class ForumTopicRepository extends BaseRepository
{
    ...
    public function getTopicsByCategories(array $categoryIds, int $limit = 20, int $offset = 0, string $filter = 'recent'): array
    {
        ...
        $whereParts = ['t.is_approved = 1'];
        if ($filter === 'comments') {
            $whereParts[] = 't.last_reply_at IS NOT NULL';
        }

        $placeholders = [];
        foreach ($categoryIds as $index => $categoryId) {
            $placeholders[] = ':category_id_' . $index;
        }
        $whereParts[] = 't.id_category IN (' . implode(', ', $placeholders) . ')';

        $orderBy = match ($filter) {
            'comments' => 'ORDER BY t.is_pinned DESC, t.last_reply_at DESC, t.created_at DESC',
            'popular' => 'ORDER BY t.is_pinned DESC, t.likes_count DESC, t.replies_count DESC, t.views_count DESC',
            'views' => 'ORDER BY t.is_pinned DESC, t.views_count DESC, t.created_at DESC',
            default => 'ORDER BY t.is_pinned DESC, t.created_at DESC',
        };
        ...
        foreach ($categoryIds as $index => $categoryId) {
            $this->db->bind(':category_id_' . $index, $categoryId);
        }
        $this->db->bind(':limit', $limit);
        $this->db->bind(':offset', $offset);
    ...
    public function countByCategories(array $categoryIds, string $filter = 'recent'): int
    {
        $categoryIds = array_values(array_unique(array_map('intval', $categoryIds)));
        if (empty($categoryIds)) {
            return 0;
        ...
        if ($filter === 'comments') {
            $whereParts[] = 'last_reply_at IS NOT NULL';
        }
        ...
        $whereParts[] = 'id_category IN (' . implode(', ', $placeholders) . ')';
