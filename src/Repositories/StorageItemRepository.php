...
class StorageItemRepository extends BaseRepository
{
    ...
    public function searchByName(int $userId, string $term): array
    {
        $this->db->query("
            ...
            WHERE (i.name LIKE :term OR c.name LIKE :term) and c.id_owner = :userId
    ...
    public function getAllByOwner(int $userId): array
    {
        return $this->searchByName($userId, '');
    }
