...
class OrdersRepository extends BaseRepository
{
   ...
   public function getOrdersByInvitation(int $userId): array
    ...
    public function getOrdersByInvitationForOwner(int $userId, int $ownerId): array
    {
        $sql = "
            SELECT DISTINCT o.*, osi.is_confirmed, i.company_name as institution_name
            FROM orders o
        ...
        $this->db->query(sql: $sql);
        $this->db->bind(param: ":userId", value: $userId);
        $this->db->bind(param: ":ownerId", value: $ownerId);
...
public function getOrdersForClientByOwner(int $clientId, int $ownerId): array
{
...
public function getOneByIdAndOwner(int $id, int $institutionOwnerId): ?object
    ...
    /**
     * Próximo evento (orden) por empresa (id_owner o id_user legacy).
     */
    public function getNextUpcomingByOwner(int $ownerId): ?object
    {
        $sql = "SELECT * FROM `{$this->table}` 
                WHERE (id_owner = :owner_id OR (id_owner IS NULL AND id_user = :owner_id)) 
                AND event_date >= CURDATE() AND is_archived = 0 
    ...
    /**
     * Próximo evento para un cliente (sus órdenes).
     */
    public function getNextUpcomingByClient(int $clientId): ?object
    {
        $sql = "SELECT * FROM `{$this->table}` 
    ...
    /**
     * Cantidad de eventos próximos por empresa.
     */
    public function getUpcomingCountByOwner(int $ownerId): int
    {
    ...
    /**
     * Cantidad de eventos próximos para un cliente.
     */
    public function getUpcomingCountByClient(int $clientId): int
    {
        $sql = "SELECT COUNT(*) AS total FROM `{$this->table}` 
                WHERE id_client = :client_id 
                AND event_date >= CURDATE() AND is_archived = 0";
    ...
    /**
     * Cantidad de órdenes (eventos) por empresa en un mes dado.
     */
    public function getOrdersCountByOwnerInMonth(int $ownerId, int $year, int $month): int
    {
    ...
    /**
     * Total de órdenes no archivadas por empresa (id_owner o id_user legacy).
     */
    public function getTotalOrdersCountByOwner(int $ownerId): int
    {
        $sql = "SELECT COUNT(*) AS total FROM `{$this->table}` 
                WHERE (id_owner = :owner_id OR (id_owner IS NULL AND id_user = :owner_id)) 
                AND is_archived = 0";
