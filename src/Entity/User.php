...
public const EXTERNAL_USER_LEVEL = [4,5,6];
     ...
     * @param string|null $systemLanguage
 
     */
 
    public function __construct(int $id, string $name, string $lastname, string $email, string $password, ?string $phone, ?int $phoneValidation, ?string $membershipDueDate, ?int $level, ?string $phoneCode, ?int $id_owner, $permissions2, ?int $allowChatWithClients , string $membership_type, string $is_active,?string $googleId = null, ?string $googleToken = null, ?string $uiLanguage = null, ?string $systemLanguage = null)
    {
        $this->id = $id;
        $this->name = $name;
    ...
    public function setSystemLanguage(?string $systemLanguage): void
    {
        $this->systemLanguage = $systemLanguage ?? 'en';
    }
