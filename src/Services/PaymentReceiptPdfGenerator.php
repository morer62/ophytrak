...
class PaymentReceiptPdfGenerator
{
    public static function generateAndSave(int $orderId, ?int $suborderId, float $amountPaid, string $paymentMethodLabel = 'Stripe', string $paymentConcept = ''): string
    {
        ...
        if ($isSub) {
            ...
            foreach ($subItems as $it) {
                ...
                $rows .= '<tr>
                    ...
                    <td style="text-align:right;">' . $currencyCode . ' ' . number_format($unit, 2) . '</td>
                    <td style="text-align:right;">' . $currencyCode . ' ' . number_format($line, 2) . '</td>
                </tr>';
        ...
        foreach ($payments as $payment) {
            ...
            $paymentHistory .= $paymentDate . ' (' . $paymentMethod . ') ' . $currencyCode . ' ' . number_format((float)$payment->amount, 2) . '<br>';
        }
        ...
        // Determine payment status (traducir después de establecer locale)
        $depositStatusTranslated = $depositAmount > 0 ? TranslationService::trans('planner_hub.pdf_paid') : TranslationService::trans('planner_hub.pdf_unpaid');
        $balanceStatusTranslated = $remainingBalance > 0 ? TranslationService::trans('planner_hub.pdf_unpaid') : TranslationService::trans('planner_hub.pdf_paid');
        ...
        if (empty($paymentConcept)) {
            if ($isSub) {
                ...
                if ($remainingBalance <= 0) {
                    $paymentConcept = TranslationService::trans('planner_hub.full_payment');
                } else {
                    $paymentConcept = TranslationService::trans('planner_hub.first_installment_payment');
                }
            }
        }
        ...
        <body>
            ...
            <div class="header">
                ...
                <div class="invoice-info">
                    <div class="invoice-number">' . TranslationService::trans('planner_hub.pdf_invoice_number') . '</div>
                    <div>' . TranslationService::trans('planner_hub.pdf_issue_date') . '</div>
            ...
            <div class="title">' . TranslationService::trans('planner_hub.pdf_order_summary') . '</div>
            <div class="subtitle">' . TranslationService::trans('planner_hub.pdf_thank_you_business') . '</div>
            ...
            <div class="grid-4">
                <div class="grid-row">
                    ...
                    <div class="grid-cell">
                        <div class="grid-header">' . TranslationService::trans('planner_hub.pdf_invoice_information') . '</div>
                        <div class="grid-content">' . TranslationService::trans('planner_hub.pdf_created_on') . ' ' . date("F j, Y") . '</div>
                        <div class="grid-content">' . $currencyCode . ' ' . number_format($total, 2) . '</div>
                        <div class="grid-content">' . TranslationService::trans('planner_hub.pdf_service_date') . ': ' . date("F j, Y", strtotime($order->event_date)) . '</div>
                    </div>
                    <div class="grid-cell">
                        <div class="grid-header">' . TranslationService::trans('planner_hub.pdf_deposit') . '</div>
                        <div class="grid-content">' . TranslationService::trans('planner_hub.pdf_to_pay_on') . ' ' . date("M j, Y", strtotime($order->event_date)) . '</div>
                        <div class="grid-content">' . $currencyCode . ' ' . number_format($depositAmount, 2) . '</div>
                    </div>
                    <div class="grid-cell">
                        <div class="grid-header">' . TranslationService::trans('planner_hub.pdf_balance') . '</div>
                        <div class="grid-content">' . TranslationService::trans('planner_hub.pdf_to_pay_on') . ' ' . date("M j, Y", strtotime($order->event_date)) . '</div>
                        <div class="grid-content">' . $currencyCode . ' ' . number_format($remainingBalance, 2) . '</div>
                    </div>
                </div>
            </div>
            ...
            <table>
                <thead>
                    <tr>
                        <th style="text-align:left;">' . TranslationService::trans('planner_hub.pdf_articles') . '</th>
                        <th>' . TranslationService::trans('planner_hub.pdf_quantity') . '</th>
                        <th>' . TranslationService::trans('planner_hub.pdf_price') . '</th>
                        <th>' . TranslationService::trans('planner_hub.pdf_amount') . '</th>
                    </tr>
                </thead>
                <tbody>
                    ' . $rows . '
            ...
            <div class="totals-table">
                <table style="width: 100%;">
                    <tr><td>' . TranslationService::trans('planner_hub.pdf_subtotal') . '</td><td style="text-align:right;">' . $currencyCode . ' ' . number_format($subtotal, 2) . '</td></tr>';
        ...
        if ($discountAmount > 0) {
            if ($discountType === 'percentage') {
                ...
                $discountLabel = TranslationService::trans('planner_hub.pdf_discount') . ' (' . $originalPercentage . '%)';
            } else {
                $discountLabel = TranslationService::trans('planner_hub.pdf_discount');
            }
            $html .= '<tr><td>' . $discountLabel . '</td><td style="text-align:right;">-' . $currencyCode . ' ' . number_format($discountAmount, 2) . '</td></tr>';
        }
        
        $html .= '                    <tr><td>' . TranslationService::trans('planner_hub.pdf_tax') . '</td><td style="text-align:right;">' . $currencyCode . ' ' . number_format($taxAmount, 2) . '</td></tr>';
        
        if ($tipAmount > 0) {
            $html .= '                    <tr><td>' . TranslationService::trans('planner_hub.pdf_tip') . ' (' . $tipPercentage . '%)</td><td style="text-align:right;">' . $currencyCode . ' ' . number_format($tipAmount, 2) . '</td></tr>';
        }
        
        $html .= '                    <tr class="total-row"><td>' . TranslationService::trans('planner_hub.pdf_total_to_pay') . '</td><td style="text-align:right;">' . $currencyCode . ' ' . number_format($total, 2) . '</td></tr>
                </table>
            </div>

            <div class="footer">
                <div>' . TranslationService::trans('planner_hub.pdf_page_of', ['page' => 1, 'total' => 2]) . '</div>
            </div>
            ...
            <div class="page-break">
                ...
                <div class="payment-section">
                    ...
                    <div class="payment-grid">
                        <div class="payment-grid-row">
                            <div class="payment-grid-cell">
                                <div class="payment-header">' . TranslationService::trans('planner_hub.pdf_deposit') . '</div>
                                <div class="payment-amount">' . $currencyCode . ' ' . number_format($depositAmount, 2) . '</div>
                                <div class="payment-status">' . $depositStatusTranslated . ' • ' . TranslationService::trans('planner_hub.pdf_due_on') . ' ' . date("M j, Y", strtotime($order->event_date)) . '</div>
                            </div>
                            <div class="payment-grid-cell">
                                <div class="payment-header">' . TranslationService::trans('planner_hub.pdf_balance') . '</div>
                                <div class="payment-amount">' . $currencyCode . ' ' . number_format($remainingBalance, 2) . '</div>
                                <div class="payment-status">' . $balanceStatusTranslated . ' • ' . TranslationService::trans('planner_hub.pdf_due_on') . ' ' . date("M j, Y", strtotime($order->event_date)) . '</div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="payment-history">
                        <h3>' . TranslationService::trans('planner_hub.pdf_payments') . '</h3>
                        <div style="font-size: 9px; line-height: 1.4;">' . 
                        ($advancesTotal > 0 ? date("M j, Y") . ' (' . TranslationService::trans('planner_hub.pdf_advance') . ') ' . $currencyCode . ' ' . number_format($advancesTotal, 2) . '<br>' : '') .
                        ($paymentHistory ?: date("M j, Y") . ' (' . TranslationService::trans('planner_hub.pdf_effective') . ') ' . $currencyCode . ' ' . number_format($depositAmount, 2)) . '
                        </div>
                    </div>
                    
                    <!-- Payment Concept -->
                    <div style="margin-top: 20px; padding: 15px; background: #f8f9fa; border: 1px solid #d6dde3;">
                        <div style="font-weight: bold; font-size: 11px; margin-bottom: 5px;">' . TranslationService::trans('planner_hub.pdf_payment_concept') . '</div>
                        <div style="font-size: 10px; color: #152026;">' . htmlspecialchars($paymentConcept) . '</div>
                        <div style="font-size: 9px; color: #6b7a85; margin-top: 3px;">' . TranslationService::trans('planner_hub.pdf_amount_label') . ': ' . $currencyCode . ' ' . number_format($amountPaid, 2) . ' ' . TranslationService::trans('planner_hub.pdf_via') . ' ' . htmlspecialchars($paymentMethodLabel) . '</div>
                    </div>
                    
                    <!-- Payment Summary and Method -->
                    ' . self::generatePaymentDetailsSection($payments, $amountPaid, $totalPaid, $order->id_owner, $currencyCode) . '
                </div>

                <div class="footer">
                    <div>' . TranslationService::trans('planner_hub.pdf_page_of', ['page' => 2, 'total' => 2]) . '</div>
                </div>
            </div>
        </body>
        </html>';
    ...
    private static function generatePaymentDetailsSection(array $payments, float $amountPaid, float $totalPaid, ?int $ownerId = null, string $currencyCode = 'USD'): string
    {
        ...
        if ($latestPayment) {
            ...
            $paymentSummaryHtml = '
                <div style="margin-top: 20px; padding: 15px; background: #f8f9fa; border: 1px solid #d6dde3;">
                    <div style="font-weight: bold; font-size: 11px; margin-bottom: 10px; color: #152026;">' . TranslationService::trans('planner_hub.pdf_payment_summary') . '</div>
                    <div style="font-size: 9px; color: #152026; margin-bottom: 5px;">' . TranslationService::trans('planner_hub.pdf_payments_made') . ': ' . $currencyCode . ' ' . number_format($totalPaid, 2) . '</div>
                    <div style="font-size: 9px; color: #152026;">' . TranslationService::trans('planner_hub.pdf_total_paid') . ': ' . $currencyCode . ' ' . number_format($totalPaid, 2) . '</div>
                </div>';
            ...
            if (!empty($cardBrand) || !empty($cardLast4)) {
                ...
                <div style="margin-top: 15px; padding: 15px; background: #f8f9fa; border: 1px solid #d6dde3;">
                    <div style="font-weight: bold; font-size: 11px; margin-bottom: 10px; color: #152026;">' . TranslationService::trans('planner_hub.pdf_payment_method') . '</div>
                    <div style="font-size: 9px; color: #152026; margin-bottom: 5px;">' . TranslationService::trans('planner_hub.pdf_type') . ': ' . htmlspecialchars($cardType) . '</div>
                    <div style="font-size: 9px; color: #152026; margin-bottom: 5px;">' . TranslationService::trans('planner_hub.pdf_number') . ': ' . htmlspecialchars($cardNumber) . '</div>';
                
                if ($cardExp) {
                    $paymentMethodHtml .= '<div style="font-size: 9px; color: #152026;">' . TranslationService::trans('planner_hub.pdf_expires_on') . ': ' . htmlspecialchars($cardExp) . '</div>';
                }
                
                $paymentMethodHtml .= '</div>';
            }

            // Billing address (if present for this payment)
            $billingAddress = $latestPayment->billing_address ?? null;
            if (!empty(trim((string)$billingAddress))) {
                $paymentMethodHtml .= '
                <div style="margin-top: 15px; padding: 15px; background: #f8f9fa; border: 1px solid #d6dde3;">
                    <div style="font-weight: bold; font-size: 11px; margin-bottom: 5px; color: #152026;">' . TranslationService::trans('planner_hub.billing_address') . '</div>
                    <div style="font-size: 9px; color: #152026; line-height: 1.3;">' . htmlspecialchars(trim($billingAddress)) . '</div>
                </div>';
            }
        }
