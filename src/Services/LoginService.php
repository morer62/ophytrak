...
class LoginService
{
    ...
    private static array $activeChecks = [];
    ...
    public function authenticate($email, $password): void
    {
        ...
        // Detectar idioma si no está guardado
        $uiLanguage = $user->ui_language ?? null;
        $systemLanguage = $user->system_language ?? null;
        
        if (!$uiLanguage) {
            // Detectar del navegador y guardar
            $detectedLocale = TranslationService::detectBrowserLocale();
            $uiLanguage = $detectedLocale && TranslationService::isSupportedLocale($detectedLocale)
                ? TranslationService::normalizeLocale($detectedLocale)
                : 'en';
            
            // Guardar en BD
            $systemLanguage = TranslationService::normalizeSupportedLocale($systemLanguage ?? 'en');
            $userRepository->updateUserLanguages($user->id, $uiLanguage, $systemLanguage);
        } else {
            $uiLanguage = TranslationService::normalizeSupportedLocale($uiLanguage);
            $systemLanguage = TranslationService::normalizeSupportedLocale($systemLanguage ?? 'en');
        }
        ...
        // Establecer idioma en TranslationService
    ...
    /** Fuerza leer usuario desde $_SESSION en la próxima getSession (útil para APIs) */
    ...
    if (self::$cachedSession !== null) {
        // Verificar que la sesión sigue coincidiendo (el usuario pudo haber cambiado en otra pestaña/request)
        if (isset($_SESSION['user']) && $_SESSION['user'] instanceof User
            && $_SESSION['user']->getId() === self::$cachedSession->getId()) {
            if (!self::isDatabaseUserActive(self::$cachedSession->getId())) {
                self::logout();
                return null;
            }
            return self::$cachedSession;
        }
        // Sesión cambiada o cerrada - invalidar caché
        self::$cachedSession = null;
    }

    if (isset($_SESSION['user'])) {
        if ($_SESSION['user'] instanceof User && !self::isDatabaseUserActive($_SESSION['user']->getId())) {
            self::logout();
            return null;
        }
    ...
    if (isset($_COOKIE['vnv_autologin'])) {
        ...
        if (isset($data['id'])) {
            ...
            if ($user && (int)$user->is_active !== 0) {
                ...
                $uiLanguage = TranslationService::normalizeSupportedLocale($user->ui_language ?? 'en');
                $systemLanguage = TranslationService::normalizeSupportedLocale($user->system_language ?? 'en');
    ...
    private static function isDatabaseUserActive(int $userId): bool
    {
        if (array_key_exists($userId, self::$activeChecks)) {
            return self::$activeChecks[$userId];
        }
        $user = (new UserRepository())->getOneWithoutOwnership(['id' => $userId]);
        self::$activeChecks[$userId] = $user && (int)($user->is_active ?? 1) !== 0;
        return self::$activeChecks[$userId];
    }
    ...
    public static function logout(): void {
        unset($_SESSION['user']);
        self::$cachedSession = null; // Limpiar caché para evitar devolver usuario anterior
    ...
    public static function reloadUserPermissions(?int $institutionId = null): void
    {
        ...
        $userData = $userRepository->getOneWithoutOwnership(['id' => $user->getId()]);
        ...
        $uiLanguage = TranslationService::normalizeSupportedLocale($userData->ui_language ?? 'en');
        $systemLanguage = TranslationService::normalizeSupportedLocale($userData->system_language ?? 'en');
   ...
   public static function validateToken(string $token): ?User
    {
        ...
        $data = $repo->getOneWithoutOwnership(["api_token" => $token]);

        if (!$data || (int)($data->is_active ?? 1) === 0) {
            return null;
        }

        $permissions = $userRoleRepository->getUserRoleAndPermissions($data->id);

        $uiLanguage = TranslationService::normalizeSupportedLocale($data->ui_language ?? 'en');
        $systemLanguage = TranslationService::normalizeSupportedLocale($data->system_language ?? 'en');
