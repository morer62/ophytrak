...
class ReminderService
{
    ...
    private function sendContractReminder($order): array
    {
        try {
            ...
            $emailService = $this->getEmailServiceForOrder($order);
            $emailSent = $emailService->sendSimpleEmail(
                $order->email,
                $subject,
                $body,
                true
    ...
    /**
     * Resolve EmailService using the owner/user of the order so that
     * reminders use the same SMTP credentials as other panel emails.
     */
    private function getEmailServiceForOrder($order): EmailService
    {
        // In orders and suborders queries, id_user represents the owner/user
        // who created the order and whose SMTP settings should be used.
        $ownerId = isset($order->id_user) ? (int) $order->id_user : null;
