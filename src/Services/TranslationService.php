...
class TranslationService
{
    ...
    {
        $requestLocale = $_GET['locale'] ?? $_POST['locale'] ?? null;
        if (is_string($requestLocale)) {
            $locale = self::normalizeSupportedLocale($requestLocale, '');
            if ($locale !== '') {
        ...
        if (is_string($sessionLocale)) {
            $locale = self::normalizeSupportedLocale($sessionLocale, '');
            if ($locale !== '') {
                self::setLocale($locale);
                return $locale;
            }
        ...
        $cookieLocale = $_COOKIE['app_locale'] ?? null;
        if (is_string($cookieLocale)) {
            $locale = self::normalizeSupportedLocale($cookieLocale, '');
            if ($locale !== '') {
                self::setLocale($locale);
                return $locale;
            }
        ...
        if ($user && method_exists($user, 'getUiLanguage')) {
            $locale = self::normalizeSupportedLocale((string) $user->getUiLanguage(), '');
            if ($locale !== '') {
                self::setLocale($locale);
                return $locale;
            }
    ...
    public static function setLocale(string $locale): bool
    {
        ...
        // Tambien establecer cookie para sincronizacion con localStorage
    ...
    /**
     * Obtiene una traduccion especifica
     */
    private static function getTranslation(string $key, string $locale): string
    {
        // Cargar el archivo de traduccion si no esta cargado
        if (!isset(self::$loadedFiles[$locale])) {
            self::loadTranslations($locale);
        }

        // Buscar la clave (soporta notacion de punto para arrays anidados)
        $keys = explode('.', $key);
    ...
    public static function getSystemLanguage(): string
    {
        ...
        // Fallback al idioma de la UI o ingles
