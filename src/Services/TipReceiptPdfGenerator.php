...
class TipReceiptPdfGenerator
{
    public static function generateAndSave(int $orderId, float $tipAmount, string $paymentMethodLabel = 'Credit Card', ?array $cardDetails = null): ?string
    {
        ...
        $html .= '
            ...
            <div class="receipt-title">' . TranslationService::trans('planner_hub.pdf_tip_receipt') . '</div>

            <div class="info-section">
                <div class="info-row">
                    <div class="info-label">' . TranslationService::trans('planner_hub.pdf_receipt_date') . ':</div>
                    <div class="info-value">' . $timestamp . '</div>
                </div>
                <div class="info-row">
                    <div class="info-label">' . TranslationService::trans('planner_hub.pdf_order_id') . ':</div>
                    <div class="info-value">VNV-341' . $order->id . '</div>
                </div>
                <div class="info-row">
                    <div class="info-label">' . TranslationService::trans('planner_hub.pdf_client_name') . ':</div>
                    <div class="info-value">' . htmlspecialchars($client->name . ' ' . $client->lastname) . '</div>
                </div>
                <div class="info-row">
                    <div class="info-label">' . TranslationService::trans('planner_hub.pdf_event_date') . ':</div>
                    <div class="info-value">' . date("F j, Y", strtotime($order->event_date)) . '</div>
                </div>
                <div class="info-row">
                    <div class="info-label">' . TranslationService::trans('planner_hub.pdf_order_total') . ':</div>
                    <div class="info-value">' . $currencyCode . ' ' . number_format($orderTotal, 2) . '</div>
                </div>
            </div>

            <div class="amount-box">
                <div class="amount-label">' . TranslationService::trans('planner_hub.pdf_tip_amount') . '</div>
                <div class="amount-value">' . $currencyCode . ' ' . number_format($tipAmount, 2) . '</div>
            </div>

            <div class="payment-details">
                <h3>' . TranslationService::trans('planner_hub.pdf_payment_information') . '</h3>
                <div class="info-row">
                    <div class="info-label">' . TranslationService::trans('planner_hub.pdf_payment_method') . ':</div>
        ...
        if ($cardDetails && !empty($cardDetails['last4'])) {
            $html .= '
                <div class="info-row">
                    <div class="info-label">' . TranslationService::trans('planner_hub.pdf_card_type') . ':</div>
                    <div class="info-value">' . htmlspecialchars(ucfirst($cardDetails['brand'] ?? TranslationService::trans('planner_hub.pdf_card'))) . '</div>
                </div>
                <div class="info-row">
                    <div class="info-label">' . TranslationService::trans('planner_hub.pdf_card_number') . ':</div>
            ...
            if (!empty($cardDetails['exp_month']) && !empty($cardDetails['exp_year'])) {
                $html .= '
                <div class="info-row">
                    <div class="info-label">' . TranslationService::trans('planner_hub.pdf_expires') . ':</div>
        ...
        $html .= '
                <div class="info-row">
                    <div class="info-label">' . TranslationService::trans('planner_hub.pdf_transaction_status') . ':</div>
                    <div class="info-value" style="color: #198754; font-weight: bold;">' . TranslationService::trans('planner_hub.pdf_completed') . '</div>
                </div>
            </div>

            <div class="thank-you">
                <div class="thank-you-text">' . TranslationService::trans('planner_hub.pdf_thank_you_generosity') . '</div>
                <p style="margin: 10px 0 0 0; color: #6b7a85; font-size: 10px;">
                    ' . TranslationService::trans('planner_hub.pdf_gratuity_appreciated') . '
                </p>
            </div>
