    ...
    public static function generateAndSave(int $orderId): string
    {
        ...
        foreach ($items as $it) {
            ...
            $rows .= '<tr>
                ...
                <td style="text-align:right;">' . $currencyCode . ' ' . number_format($unitPrice, 2) . '</td>
                <td style="text-align:right;">' . $currencyCode . ' ' . number_format($lineSubtotal, 2) . '</td>
            </tr>';
            ...
            <table style="margin-top:8px;">
                <tr class="totals"><td style="width:70%;border:none;"></td><td style="width:30%;border:none;">
                    <table style="width:100%;">
                        <tr><td>Subtotal</td><td class="right">' . $currencyCode . ' ' . number_format($subtotal, 2) . '</td></tr>
                        <tr><td>Discount (' . ($discountType === 'percentage' ? ($discountValue . '%') : ($currencyCode . ' ' . number_format($discountValue, 2))) . ')</td><td class="right">-' . $currencyCode . ' ' . number_format($discountAmount, 2) . '</td></tr>
                        <tr><td>Tax (' . number_format($taxPercent, 2) . '%)</td><td class="right">' . $currencyCode . ' ' . number_format($taxAmount, 2) . '</td></tr>
                        <tr><td><b>Total</b></td><td class="right"><b>' . $currencyCode . ' ' . number_format($total, 2) . '</b></td></tr>';
        
        if ($advancesTotal > 0) {
            $html .= '<tr><td>Advances Applied</td><td class="right">- ' . $currencyCode . ' ' . number_format($advancesTotal, 2) . '</td></tr>';
        }
        
        if ($paymentsTotal > 0) {
            $html .= '<tr><td>Payments Made</td><td class="right">- ' . $currencyCode . ' ' . number_format($paymentsTotal, 2) . '</td></tr>';
        }
        
        $html .= '<tr><td><b>Total Due</b></td><td class="right"><b>' . $currencyCode . ' ' . number_format($effectiveTotal, 2) . '</b></td></tr>
                    </table>
                </td></tr>
            </table>

            <div class="grid" style="margin-top:8px;">
                <div class="cell"><b>Deposit</b>' . $currencyCode . ' ' . number_format($deposit, 2) . '</div>
                <div class="cell"><b>Balance</b>' . $currencyCode . ' ' . number_format($balance, 2) . '</div>
            </div>
