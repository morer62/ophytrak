...
All active Ophyra businesses are Level 2. The former split `Level 2 = Venue` and `Level 3 = Vendor / Service Provider` is not active anymore. Business type must be stored as Business Profile/module data, not as `users.level`.
...
Level 3 is annulled for active Ophyra business modeling.

Do not create Level 3 users for new businesses. Do not use Level 3 to represent vendors, service providers, venues, stores or any other business type. Legacy Level 3 code may remain only for backward compatibility until intentionally retired.

If a user needs a business account, create or route them to Level 2. If the business needs a type/category, store it in Business Profile fields and module configuration.
...
* Do not mutate `users.level` just to switch dashboards or contexts.
* If a Level 4 user wants to create a business, send the user through the business signup/onboarding flow.
* Keep existing Level 4 company assignments in `user_institutions`.
* Navigation for Level 4 must be generated from the selected institution plus approved role/module permissions.
...
* Do not mutate `users.level` just to show Business Owner or Team views.
* A Level 5 user who wants a business account should use the business signup/onboarding flow.
* Service and Store order lists must show all purchases for the logged-in client across companies.
* Each order row must identify the selling business using `orders.id_owner` or `store_orders.id_owner` joined to `institution_profile`.
...
## Multimedia / Music Sessions

The current `music_sessions` table is owner-scoped through:

```text
music_sessions.id_user
```

Rules:

* Level 5 may see music sessions for the selected company context.
* Public multimedia search should pass an explicit owner scope when it is used for a specific brand or business.
* The public route supports `owner_id` and the `PUBLIC_MULTIMEDIA_OWNER_ID` environment value for owner scoping.
* Future work can add a deliberate `project_key` or `site_key` column, but public pages should not assume all active sessions belong to Ophyra globally.
