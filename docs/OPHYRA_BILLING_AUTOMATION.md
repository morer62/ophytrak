...
user_id
ophyra_base=1
module_slug=store_logistics
```

For multiple paid modules:

```text
module_slugs=service_operations,store_logistics,advanced_storage_qr_inventory,ai_advisor,ticket_sales_rsvp
```

Legacy-compatible slugs are still accepted where the current implementation uses them: `services`, `store_delivery_tracking`, `inventory_storage`, and `tickets_rsvp`.
...
For self-service Level 2 module activation, the user must pass through the review route before any charge:

```text
/panel/membership/modules/review?module_slug={module_slug}
```

The marketplace button must link to review or continue payment. It must not activate a paid module directly.

The review page:

1. Shows the module, base USD amount, selected billing currency, converted amount when a real exchange rate exists, renewal date and unlocked features.
2. Marks the module `PENDING` with `billing_status = checkout_started` only after the user confirms.
3. Charges the saved Stripe customer/card only after the user clicks Confirm and pay.
4. Activates the module only after Stripe returns a confirmed paid charge, or after a later verified webhook/invoice event.
5. Marks failures as inactive/payment_failed and keeps Base Profile available.

For `invoice.paid` or paid `checkout.session.completed`, the webhook service attempts to:
...
Manual Level 1 activation is separate from paid activation. When the optional hardening SQL has been run, `user_modules.activation_source`, `activated_by_admin_id`, `activation_reason`, `current_period_start` and `last_payment_id` preserve that distinction.
