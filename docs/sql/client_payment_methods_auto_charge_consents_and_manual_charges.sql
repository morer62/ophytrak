-- Ophyra saved client payment methods, auto-charge consent, and manual authorized charges
-- Manual SQL only. Do not convert this file into a migration without review.
--
-- Scope:
-- - Gateway-agnostic client payment method references.
-- - Explicit future/automatic charge consent per business/client/provider.
-- - Auditable Level 1 / Level 2 manual authorized charge attempts.
--
-- Safety:
...
CREATE TABLE IF NOT EXISTS `authorized_manual_charge_logs` (
  ...
  `charged_by_level` varchar(20) NOT NULL,
  ...
  KEY `idx_amcl_business_order` (`id_user_business`, `order_id`),
  KEY `idx_amcl_business_store_order` (`id_user_business`, `store_order_id`),
  KEY `idx_amcl_client` (`client_id`),
  KEY `idx_amcl_method` (`saved_payment_method_id`),
  KEY `idx_amcl_consent` (`consent_id`),
  KEY `idx_amcl_status` (`status`),
  CONSTRAINT `fk_amcl_saved_method`
    FOREIGN KEY (`saved_payment_method_id`) REFERENCES `client_saved_payment_methods` (`id`)
    ON DELETE SET NULL,
  CONSTRAINT `fk_amcl_consent`
    FOREIGN KEY (`consent_id`) REFERENCES `client_auto_charge_consents` (`id`)
    ON DELETE SET NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
-- payment_providers_credentials.provider_type is currently an enum('stripe','square','paypal').
-- Do not alter it blindly. If/when manual and future providers are configured in the same table,
-- review dashboard behavior first and then run an explicit ALTER TABLE approved for production.
