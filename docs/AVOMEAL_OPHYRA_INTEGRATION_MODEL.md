...
+ Meal subscriptions
+ Delivery zones
+ Avomeal site scope where shared ownership requires it
```
...
Avomeal mobile now explicitly sends:
...
This means Ophyra must distinguish:

```text
id_owner / id_user_business = operational owner
site_key / brand_key = public site or brand identity
site_visibility = whether a record appears on that public site
brand_site_settings = per-site settings when available
```

Not every record owned by owner `2` should appear on Avomeal. Avomeal public Store, cart, checkout, subscriptions and mobile WebView routes should filter by owner plus Avomeal site visibility when the schema supports it.

During the current review, `VNV_Gourmet` web code appeared mostly owner-scoped and still contains legacy `VNV Gourmet` branding in some templates. Treat full `site_key=avomeal` backend enforcement as a compatibility gap until verified in the active database/code path.

## Growth Hub Integration

Ophyra Growth Hub can manage SEO planning and central CMS drafts for Avomeal with:

```text
site_key=avomeal
domain=avomeal.com
Search Console property=sc-domain:avomeal.com
```

Current Search Console status is connected, but the latest verified import returned no rows. That should be displayed as `connected / no rows returned`, not as an integration failure.

Avomeal Growth Hub content should focus on food/store demand such as catering, meal prep, office lunch, weekly menus, delivery zones and subscriptions. It must not mix VNV Events event operations or VNV Events public content.

Published Growth Hub content for Avomeal should be consumed by `site_key=avomeal` only.

## Required Growth Hub Sitemap Receiver

Avomeal must implement the production receiver endpoint described in `GROWTH_HUB_BRAND_SITE_ADAPTATION_GUIDE.md`:

```text
POST /api/ophyra/growth-hub/sitemap-refresh
```

When Ophyra marks `avomeal` as `production`, every published, updated or archived Growth Hub page should trigger this endpoint. Avomeal must rebuild `https://avomeal.com/sitemap.xml` from:

- existing Avomeal public routes;
- Ophyra Growth Hub published routes for `site_key=avomeal`;
- only canonical, indexable URLs that render `200 OK`.

If Ophyra is set to `development`, Avomeal must not rebuild the production sitemap from local preview URLs.
...
* If VNV Events and Avomeal share owner scope, payment-provider lookup may need site-level configuration before both brands can safely use different processors.
