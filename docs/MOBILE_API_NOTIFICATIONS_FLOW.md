...
Use these contracts before editing mobile/API behavior:
...
Avomeal mobile also sends site identity:
...
That site key must be preserved for Store, cart, checkout, orders and subscriptions where backend/site filtering supports it.
...
Brand/site scope is not consistently sent by existing apps to this endpoint. Future notification routing should add scope fields without removing the existing accepted token fields.
...
Avomeal app currently opens Store/cart/checkout/orders/subscriptions through WebView, not native Store APIs. Those routes should honor:
