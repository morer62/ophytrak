...
Marketplace ingestion is not part of the base Store + Logistics price. Mercado Libre, TikTok Business / Shop and Shopify token management, manual sync records, external status mapping and public tracking-link preparation live in the separate `marketplace_connectors` add-on, priced by:

```text
MARKETPLACE_CONNECTORS_PRICE
OPHYRA_MODULE_MARKETPLACE_CONNECTORS_MONTHLY
```
...
* Level 2 needs `marketplace_connectors` ACTIVE in `user_modules` before accessing marketplace token and manual sync tools.
* Level 1 can open Marketplace Connectors as an admin surface and select the Level 2 business owner whose connector records should be managed.
...
## Marketplace Manual Sync

The Marketplace Connectors add-on stores provider tokens separately from Store + Logistics. Until full provider API adapters are connected, the manual sync tool accepts an optional JSON payload and records it in:
...
`external_order_id`, `order_id` or `id` is required. External marketplace status is normalized into Ophyra internal statuses by `MarketplaceSyncService::normalizeExternalStatus()`. If `id_store_order` is present, the mapping is marked `MAPPED`; otherwise it remains `NEW` for manual review/import.
