...
| Post-signup onboarding | Ready for soft launch | Web signup now authenticates the new business owner automatically and redirects to `/panel/onboarding` so the user can complete Base Profile basics, see locked paid modules and continue to Billing & Modules. |
...
| New account module access | Guarded | New Level 2 accounts receive Base Profile only. Paid operational modules stay inactive unless paid, manually activated, gifted by Level 1 or explicitly trialed. Direct Level 2 operational URLs are guarded by `ModuleGuardService` and the Kernel route map. |
...
* see Base Profile as the only free included layer;
* see paid modules as recommended, locked or active;
...
* Base Profile is free and available to active Level 2 business owners.
* Paid modules are defined in `ModulesRepository::ADDON_SLUGS` with legacy operational slugs and canonical aliases.
* Service Operations is $24/month and Store + Logistics is $34/month.
...
Paid modules currently recognized by the existing implementation:
...
The dashboard quotes paid modules and displays renewal dates. Billing & Modules lets the user activate a paid module with a saved Stripe card after starting with the free Base Profile.
...
If a paid module does not have an active, valid `user_modules` row, it must be treated as inactive.
```

The following paid modules are not enabled by default:
...
8. Send `invoice.paid` with `module_slugs=store_logistics,advanced_storage_qr_inventory` or the legacy-compatible slugs `store_delivery_tracking,inventory_storage`.
