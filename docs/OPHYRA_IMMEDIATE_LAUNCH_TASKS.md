...
* Level 2 business owners can complete Base Profile for free, add cards only when needed and activate paid modules.
...
5. Ophyra Base Profile is free to start; paid modules write to `payments_all`.
...
1. Open `/panel/home` after signup.
2. Confirm Base Profile is available without billing info or a Stripe card.
3. Confirm CRM, Orders, Team, Payroll, Store, Storage, AI and Tickets are locked.
4. Open `/panel/membership/manage`.
5. Confirm paid modules are the payment trigger.
6. Add a Stripe card only when activating a paid module.
7. Confirm affiliate commission when a paid module is purchased by a referred account.
