...
* Do not reintroduce Level 3 as Vendor/Service Provider. Level 3 is annulled for active business modeling; keep only passive backward compatibility until legacy code/data are intentionally retired.
...
* Ophyra Base Profile uses `OPHYRA_BASE_PROFILE_PRICE=0`.
* Service Operations uses `SERVICE_OPERATIONS_PRICE` / `OPHYRA_MODULE_SERVICE_OPERATIONS_MONTHLY`.
* Store + Logistics uses `STORE_LOGISTICS_PRICE` / `OPHYRA_MODULE_STORE_DELIVERY_TRACKING_MONTHLY`.
* Optional modules use their approved `ADVANCED_STORAGE_QR_INVENTORY_PRICE`, `AI_ADVISOR_PRICE`, and `TICKET_SALES_RSVP_PRICE` variables.
