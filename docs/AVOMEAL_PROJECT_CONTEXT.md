...
The active public brand name is:

```text
Avomeal
```

Use `VNV Gourmet` only as repository or legacy historical context unless a specific local file intentionally still carries that name.
...
The current Avomeal mobile contract sends:
...
When Avomeal shares an owner/database with VNV Events, owner scope alone is not enough for public Store/content visibility. Avomeal public and mobile surfaces should also honor `site_key=avomeal` and `site_visibility` where supported.
